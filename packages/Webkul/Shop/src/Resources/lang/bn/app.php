<?php

return [
    'customers' => [
        'forgot-password' => [
            'already-sent' => 'ইতিমধ্যেই পাসওয়ার্ড রিসেট মেল পাঠানো হয়েছে।',
            'back' => 'সাইন ইনে ফিরে যেতে?',
            'bagisto' => 'বাগিস্তো',
            'email' => 'ইমেইল',
            'email-not-exist' => 'আমরা ঐ ইমেইল ঠিকানার সাথে কোনও ব্যবহারকারী খুঁজে পেতে পারি নি।',
            'footer' => '© কপিরাইট 2010 - :current_year, ওয়েবকুল সফটওয়্যার (ভারতে নিবন্ধিত)। সব অধিকার সংরক্ষিত।',
            'forgot-password-text' => 'আপনি যদি আপনার পাসওয়ার্ড ভুলে গিয়েছেন, তাহলে আপনি আপনার ইমেইল ঠিকানা দিয়ে এটি পুনরুদ্ধার করতে পারেন।',
            'page-title' => 'আপনি কি আপনার পাসওয়ার্ড ভুলে গিয়েছেন?',
            'reset-link-sent' => 'আমরা আপনার পুনরায় নির্ধারণ করার লিঙ্কটি ইমেইল করেছি।',
            'sign-in-button' => 'সাইন ইন করুন',
            'submit' => 'পাসওয়ার্ড পুনরুদ্ধার করুন',
            'title' => 'পাসওয়ার্ড পুনরুদ্ধার করুন',
        ],

        'reset-password' => [
            'back-link-title' => 'সাইন ইনে ফিরে যেতে',
            'bagisto' => 'বাগিস্তো',
            'confirm-password' => 'পাসওয়ার্ড নিশ্চিত করুন',
            'email' => 'রেজিস্টারড ইমেইল',
            'footer' => '© কপিরাইট 2010 - :current_year, ওয়েবকুল সফটওয়্যার (ভারতে নিবন্ধিত)। সব অধিকার সংরক্ষিত।',
            'password' => 'পাসওয়ার্ড',
            'submit-btn-title' => 'পাসওয়ার্ড রিসেট করুন',
            'title' => 'পাসওয়ার্ড রিসেট করুন',
        ],

        'login-form' => [
            'bagisto' => 'বাগিস্তো',
            'button-title' => 'সাইন ইন করুন',
            'create-your-account' => 'আপনার একাউন্ট তৈরি করুন',
            'email' => 'ইমেইল',
            'footer' => '© কপিরাইট 2010 - :current_year, ওয়েবকুল সফটওয়্যার (ভারতে নিবন্ধিত)। সব অধিকার সংরক্ষিত।',
            'forgot-pass' => 'পাসওয়ার্ড ভুলে গিয়েছেন?',
            'form-login-text' => 'আপনি যদি আপনার একাউন্ট আছে তবে আপনি আপনার ইমেইল ঠিকানা দিয়ে সাইন ইন করতে পারেন।',
            'invalid-credentials' => 'দুঃখিত, আপনার শূন্যমূল্য সত্যাপন দেওয়া গেছে না।',
            'new-customer' => 'নতুন গ্রাহক?',
            'not-activated' => 'আপনার সক্রিয়ণ অ্যাডমিনের অনুমোদন প্রয়োজন',
            'page-title' => 'গ্রাহক লগইন',
            'password' => 'পাসওয়ার্ড',
            'resend-verification' => 'ভেরিফিকেশন ইমেইল পুনরায় পাঠান',
            'show-password' => 'পাসওয়ার্ড দেখুন',
            'title' => 'সাইন ইন করুন',
            'verify-first' => 'প্রথমে আপনার ইমেইল অ্যাকাউন্ট যাচাই করুন।',
        ],

        'signup-form' => [
            'account-exists' => 'ইতিমধ্যে একটি অ্যাকাউন্ট আছে?',
            'bagisto' => 'বাগিস্তো',
            'button-title' => 'রেজিস্টার করুন',
            'click-here' => 'এখানে ক্লিক করুন',
            'confirm-pass' => 'পাসওয়ার্ড নিশ্চিত করুন',
            'email' => 'ইমেইল',
            'first-name' => 'প্রথম নাম',
            'footer' => '© কপিরাইট 2010 - :current_year, ওয়েবকুল সফটওয়্যার (ভারতে নিবন্ধিত)। সব অধিকার সংরক্ষিত।',
            'form-signup-text' => 'আমাদের স্টোরে আপনি যদি নতুন হোন তাহলে আমরা আপনাকে সদস্য হিসেবে স্বাগত জানাই।',
            'last-name' => 'শেষ নাম',
            'page-title' => 'ব্যবহারকারী হোন',
            'password' => 'পাসওয়ার্ড',
            'sign-in-button' => 'সাইন ইন করুন',
            'subscribe-to-newsletter' => 'নিউজলেটারে সাবস্ক্রাইব করুন',
            'success' => 'অ্যাকাউন্ট সফলভাবে তৈরি হয়েছে।',
            'success-verify' => 'অ্যাকাউন্ট সফলভাবে তৈরি হয়েছে, একটি যাচাই ইমেইল পাঠানো হয়েছে।',
            'terms-conditions' => 'শর্তাবলী',
            'verification-not-sent' => 'ত্রুটি! যাচাই ইমেইল পাঠানোর সময় সমস্যা, দয়া করে পরে আবার চেষ্টা করুন।',
            'verification-sent' => 'যাচাই ইমেইল পাঠানো হয়েছে',
            'verified' => 'আপনার অ্যাকাউন্ট যাচাই করা হয়েছে, এখন লগইন চেষ্টা করুন।',
            'verify-failed' => 'আমরা আপনার ইমেইল অ্যাকাউন্ট যাচাই করতে পারি নি।',
        ],

        'account' => [
            'home' => 'হোম',

            'profile' => [
                'index' => [
                    'delete' => 'মুছে ফেলুন',
                    'delete-failed' => 'গ্রাহক মুছে ফেলার সময় ত্রুটি হয়েছে।',
                    'delete-profile' => 'প্রোফাইল মুছুন',
                    'delete-success' => 'গ্রাহক সফলভাবে মুছে ফেলা হয়েছে',
                    'dob' => 'জন্ম তারিখ',
                    'edit' => 'সম্পাদনা করুন',
                    'edit-success' => 'প্রোফাইল সফলভাবে আপডেট হয়েছে',
                    'email' => 'ইমেইল',
                    'enter-password' => 'আপনার পাসওয়ার্ড দিন',
                    'first-name' => 'নামের প্রথম অংশ',
                    'gender' => 'লিঙ্গ',
                    'last-name' => 'নামের শেষ অংশ',
                    'order-pending' => 'কারণ কিছু অর্ডার বিচারাধীন বা প্রসেসিং অবস্থায় আছে, গ্রাহক অ্যাকাউন্ট মুছে ফেলা যাবে না।',
                    'title' => 'প্রোফাইল',
                    'unmatched' => 'পুরানো পাসওয়ার্ড মেলে না।',
                    'wrong-password' => 'ভুল পাসওয়ার্ড!',
                ],

                'edit' => [
                    'confirm-password' => 'পাসওয়ার্ড নিশ্চিত করুন',
                    'current-password' => 'বর্তমান পাসওয়ার্ড',
                    'dob' => 'জন্ম তারিখ',
                    'edit' => 'সম্পাদনা করুন',
                    'edit-profile' => 'প্রোফাইল সম্পাদনা করুন',
                    'email' => 'ইমেইল',
                    'female' => 'মহিলা',
                    'first-name' => 'নামের প্রথম অংশ',
                    'gender' => 'লিঙ্গ',
                    'last-name' => 'নামের শেষ অংশ',
                    'male' => 'পুরুষ',
                    'new-password' => 'নতুন পাসওয়ার্ড',
                    'other' => 'অন্যান্য',
                    'phone' => 'ফোন',
                    'save' => 'সংরক্ষণ করুন',
                    'subscribe-to-newsletter' => 'নিউজলেটারে সাবস্ক্রাইব করুন',
                ],
            ],

            'addresses' => [
                'index' => [
                    'add-address' => 'ঠিকানা যুক্ত করুন',
                    'create-success' => 'ঠিকানা সফলভাবে যুক্ত হয়েছে।',
                    'default-address' => 'ডিফল্ট ঠিকানা',
                    'default-delete' => 'ডিফল্ট ঠিকানা পরিবর্তন করা যাবে না।',
                    'delete' => 'মুছে ফেলুন',
                    'delete-success' => 'ঠিকানা সফলভাবে মুছে ফেলা হয়েছে',
                    'edit' => 'সম্পাদনা করুন',
                    'edit-success' => 'ঠিকানা সফলভাবে আপডেট হয়েছে।',
                    'empty-address' => 'আপনি এখনো আপনার অ্যাকাউন্টে কোনও ঠিকানা যুক্ত করেননি।',
                    'security-warning' => 'সন্দেহজনক কার্যক্রম পাওয়া গেছে!!!',
                    'set-as-default' => 'ডিফল্ট হিসাবে সেট করুন',
                    'title' => 'ঠিকানা',
                    'update-success' => 'ঠিকানা সফলভাবে আপডেট হয়েছে।',
                ],

                'create' => [
                    'add-address' => 'ঠিকানা যুক্ত করুন',
                    'city' => 'শহর',
                    'company-name' => 'কোম্পানির নাম',
                    'country' => 'দেশ',
                    'email' => 'ইমেইল',
                    'first-name' => 'নামের প্রথম অংশ',
                    'last-name' => 'নামের শেষ অংশ',
                    'phone' => 'ফোন',
                    'post-code' => 'পোস্ট কোড',
                    'save' => 'সংরক্ষণ করুন',
                    'select-country' => 'দেশ নির্বাচন করুন',
                    'set-as-default' => 'ডিফল্ট হিসাবে সেট করুন',
                    'state' => 'রাষ্ট্র',
                    'street-address' => 'রাস্তার ঠিকানা',
                    'title' => 'ঠিকানা',
                    'vat-id' => 'ভ্যাট আইডি',
                ],

                'edit' => [
                    'city' => 'শহর',
                    'company-name' => 'কোম্পানির নাম',
                    'country' => 'দেশ',
                    'edit' => 'সম্পাদনা করুন',
                    'email' => 'ইমেইল',
                    'first-name' => 'নামের প্রথম অংশ',
                    'last-name' => 'নামের শেষ অংশ',
                    'phone' => 'ফোন',
                    'post-code' => 'পোস্ট কোড',
                    'select-country' => 'দেশ নির্বাচন করুন',
                    'state' => 'রাষ্ট্র',
                    'street-address' => 'রাস্তার ঠিকানা',
                    'title' => 'ঠিকানা',
                    'update-btn' => 'আপডেট করুন',
                    'vat-id' => 'ভ্যাট আইডি',
                ],
            ],

            'orders' => [
                'action' => 'কর্ম',
                'action-view' => 'দেখা',
                'empty-order' => 'আপনি এখনও কোনও পণ্য অর্ডার করেন নি',
                'order' => 'অর্ডার',
                'order-date' => 'অর্ডারের তারিখ',
                'order-id' => 'অর্ডার আইডি',
                'pay-by' => 'পেমেন্ট পদ্ধতি - :method',
                'pay-via' => 'পেমেন্ট মাধ্যম',
                'subtotal' => 'সাবটোটাল',
                'title' => 'অর্ডারগুলি',
                'total' => 'মোট',

                'status' => [
                    'title' => 'অবস্থা',

                    'options' => [
                        'canceled' => 'বাতিল',
                        'closed' => 'বন্ধ',
                        'completed' => 'সম্পূর্ণ',
                        'fraud' => 'প্রতারণা',
                        'pending' => 'মুলতুলি',
                        'pending-payment' => 'মুলতুলি পেমেন্ট',
                        'processing' => 'প্রসেসিং',
                    ],

                    'canceled' => 'বাতিল',
                    'closed' => 'বন্ধ',
                    'completed' => 'সম্পন্ন',
                    'fraud' => 'প্রতারণা',
                    'pending' => 'অপেক্ষমান',
                    'pending-payment' => 'অপেক্ষমান পেমেন্ট',
                    'processing' => 'প্রক্রিয়াধীন',
                ],

                'view' => [
                    'billing-address' => 'বিলিং ঠিকানা',

                    'booking-cancellation-not-allowed' => [
                        'title' => 'বুকিং আইটেম বাতিল হবে না',
                        'description' => 'এই অর্ডারে অ-বাতিলযোগ্য হিসেবে চিহ্নিত বুকিং পণ্য রয়েছে। অর্ডার বাতিল করলে শুধুমাত্র অবশিষ্ট আইটেমগুলি বাতিল হবে; বুকিং আইটেমগুলি সক্রিয় থাকবে। বুকিং আইটেমে সহায়তার প্রয়োজন হলে সাপোর্টের সাথে যোগাযোগ করুন।',
                    ],

                    'cancel-btn-title' => 'বাতিল করুন',
                    'cancel-confirm-msg' => 'আপনি কি নিশ্চিত যে আপনি এই অর্ডারটি বাতিল করতে চান?',
                    'cancel-error' => 'আপনার অর্ডারটি বাতিল করা যাবে না।',
                    'cancel-success' => 'আপনার অর্ডারটি বাতিল হয়েছে',
                    'contact' => 'যোগাযোগ',
                    'item-invoiced' => 'আইটেম চালান করা হয়েছে',
                    'item-refunded' => 'আইটেম ফেরত দেওয়া হয়েছে',
                    'item-shipped' => 'আইটেম প্রেরণ করা হয়েছে',
                    'item-ordered' => 'আইটেম অর্ডার করা হয়েছে',
                    'order' => 'অর্ডার',
                    'order-id' => 'অর্ডার আইডি',
                    'page-title' => 'অর্ডার #:order_id',
                    'payment-method' => 'পেমেন্ট পদ্ধতি',
                    'reorder-booking-skipped' => 'পুনরায় অর্ডারের সময় বুকিং পণ্য এড়িয়ে যাওয়া হয়েছে। অনুগ্রহ করে নতুন তারিখ এবং সময় স্লট দিয়ে সেগুলি আবার বুক করুন।',
                    'reorder-btn-title' => 'পুনরায় অর্ডার করুন',
                    'shipping-address' => 'প্রেরণ ঠিকানা',
                    'shipping-method' => 'প্রেরণ পদ্ধতি',
                    'shipping-and-payment' => 'প্রেরণ এবং পেমেন্টের বিবরণ',
                    'status' => 'অবস্থা',
                    'title' => 'দেখুন',
                    'total' => 'মোট',

                    'information' => [
                        'discount' => 'ডিসকাউন্ট',
                        'excl-tax' => 'কর বাদ:',
                        'grand-total' => 'সর্বমোট',
                        'info' => 'তথ্য',
                        'item-canceled' => 'বাতিল করা আইটেম (:qty_canceled)',
                        'item-refunded' => 'ফেরত দেওয়া আইটেম (:qty_refunded)',
                        'invoiced-item' => 'চালান করা আইটেম (:qty_invoiced)',
                        'item-shipped' => 'প্রেরণ করা আইটেম (:qty_shipped)',
                        'item-status' => 'আইটেমের অবস্থা',
                        'ordered-item' => 'অর্ডার করা আইটেম (:qty_ordered)',
                        'placed-on' => 'অর্ডার দেওয়া হয়েছে',
                        'price' => 'মূল্য',
                        'product-name' => 'নাম',
                        'shipping-handling' => 'প্রেরণ এবং হ্যান্ডলিং',
                        'shipping-handling-excl-tax' => 'প্রেরণ এবং হ্যান্ডলিং (কর বাদ)',
                        'shipping-handling-incl-tax' => 'প্রেরণ এবং হ্যান্ডলিং (কর সহ)',
                        'sku' => 'SKU',
                        'subtotal' => 'সাবটোটাল',
                        'subtotal-excl-tax' => 'সাবটোটাল (কর বাদ)',
                        'subtotal-incl-tax' => 'সাবটোটাল (কর সহ)',
                        'order-summary' => 'অর্ডার সংক্ষেপ',
                        'tax' => 'কর',
                        'tax-amount' => 'করের পরিমাণ',
                        'tax-percent' => 'করের শতাংশ',
                        'total-due' => 'মোট বাকি',
                        'total-paid' => 'মোট পরিশোধ',
                        'total-refunded' => 'মোট ফেরত দেওয়া',
                    ],

                    'invoices' => [
                        'discount' => 'ডিসকাউন্ট',
                        'excl-tax' => 'কর বাদ:',
                        'grand-total' => 'সর্বমোট',
                        'individual-invoice' => 'চালান #:invoice_id',
                        'invoices' => 'চালানগুলি',
                        'price' => 'মূল্য',
                        'print' => 'প্রিন্ট',
                        'product-name' => 'নাম',
                        'products-ordered' => 'অর্ডার করা পণ্যসমূহ',
                        'qty' => 'পরিমাণ',
                        'shipping-handling-excl-tax' => 'প্রেরণ এবং হ্যান্ডলিং (কর বাদ)',
                        'shipping-handling-incl-tax' => 'প্রেরণ এবং হ্যান্ডলিং (কর সহ)',
                        'shipping-handling' => 'প্রেরণ এবং হ্যান্ডলিং',
                        'sku' => 'SKU',
                        'subtotal-excl-tax' => 'সাবটোটাল (কর বাদ)',
                        'subtotal-incl-tax' => 'সাবটোটাল (কর সহ)',
                        'subtotal' => 'সাবটোটাল',
                        'tax' => 'কর',
                        'tax-amount' => 'করের পরিমাণ',
                    ],

                    'shipments' => [
                        'individual-shipment' => 'প্রেরণ #:shipment_id',
                        'product-name' => 'নাম',
                        'qty' => 'পরিমাণ',
                        'shipments' => 'প্রেরণগুলি',
                        'sku' => 'SKU',
                        'subtotal' => 'সাবটোটাল',
                        'tracking-number' => 'ট্র্যাকিং নম্বর',
                    ],

                    'refunds' => [
                        'adjustment-fee' => 'সংশোধন ফি',
                        'adjustment-refund' => 'সংশোধন ফেরত',
                        'discount' => 'ডিসকাউন্ট',
                        'grand-total' => 'সর্বমোট',
                        'individual-refund' => 'ফেরত দেওয়া #:refund_id',
                        'no-result-found' => 'কোন রেকর্ড পাওয়া যায়নি।',
                        'order-summary' => 'অর্ডার সংক্ষেপ',
                        'price' => 'মূল্য',
                        'product-name' => 'নাম',
                        'qty' => 'পরিমাণ',
                        'refunds' => 'ফেরত দেওয়া',
                        'shipping-handling' => 'প্রেরণ এবং হ্যান্ডলিং',
                        'shipping-handling-excl-tax' => 'প্রেরণ এবং হ্যান্ডলিং (কর বাদ)',
                        'shipping-handling-incl-tax' => 'প্রেরণ এবং হ্যান্ডলিং (কর সহ)',
                        'sku' => 'SKU',
                        'subtotal' => 'সাবটোটাল',
                        'subtotal-excl-tax' => 'সাবটোটাল (কর বাদ)',
                        'subtotal-incl-tax' => 'সাবটোটাল (কর সহ)',
                        'tax' => 'কর',
                        'tax-amount' => 'করের পরিমাণ',
                    ],
                ],

                'invoice-pdf' => [
                    'bank-details' => 'ব্যাংকের বিবরণ',
                    'bill-to' => 'বিল প্রাপ্তি',
                    'contact-number' => 'যোগাযোগ নম্বর',
                    'contact' => 'যোগাযোগ',
                    'date' => 'চালানের তারিখ',
                    'discount' => 'মূল্যছাড়',
                    'excl-tax' => 'কর ব্যতিত:',
                    'grand-total' => 'সর্বমোট',
                    'invoice-id' => 'চালান আইডি',
                    'invoice' => 'চালান',
                    'order-date' => 'অর্ডারের তারিখ',
                    'order-id' => 'অর্ডার আইডি',
                    'payment-method' => 'পেমেন্ট পদ্ধতি',
                    'payment-terms' => 'পেমেন্ট শর্তাদি',
                    'price' => 'মূল্য',
                    'product-name' => 'পণ্যের নাম',
                    'qty' => 'পরিমাণ',
                    'ship-to' => 'প্রেরণ করুন',
                    'shipping-handling-excl-tax' => 'শিপিং হ্যান্ডলিং (কর ব্যতিত)',
                    'shipping-handling-incl-tax' => 'শিপিং হ্যান্ডলিং (কর সহ)',
                    'shipping-handling' => 'শিপিং হ্যান্ডলিং',
                    'shipping-method' => 'প্রেরণ পদ্ধতি',
                    'sku' => 'এসকেও',
                    'subtotal-excl-tax' => 'উপমোট (কর ব্যতিত)',
                    'subtotal-incl-tax' => 'উপমোট (কর সহ)',
                    'subtotal' => 'উপমোট',
                    'tax-amount' => 'করের পরিমাণ',
                    'tax' => 'কর',
                    'vat-number' => 'ভ্যাট নম্বর',
                ],
            ],

            'reviews' => [
                'empty-review' => 'আপনি এখনো কোনও পণ্য পর্যালোচনা করেননি',
                'title' => 'পর্যালোচনা',
            ],

            'downloadable-products' => [
                'available' => 'পাওয়া যায়',
                'completed' => 'সম্পন্ন',
                'date' => 'তারিখ',
                'download-error' => 'ডাউনলোড লিঙ্কের মেয়াদ শেষ হয়ে গিয়েছে।',
                'empty-product' => 'আপনার ডাউনলোড করার জন্য কোনও পণ্য নেই',
                'expired' => 'মেয়াদোত্তীর্ণ',
                'name' => 'ডাউনলোডযোগ্য পণ্য',
                'orderId' => 'অর্ডার আইডি',
                'payment-error' => 'এই ডাউনলোডের জন্য পেমেন্ট হয়নি।',
                'pending' => 'বিচারাধীন',
                'records-found' => 'রেকর্ড(গুলি) পাওয়া গিয়েছে',
                'remaining-downloads' => 'অবশিষ্ট ডাউনলোড',
                'status' => 'স্থিতি',
                'title' => 'শিরোনাম',
                'unlimited' => 'সীমাহীন',
            ],

            'wishlist' => [
                'color' => 'রং',
                'delete-all' => 'সব মুছুন',
                'empty' => 'কোনও পণ্যটি ইচ্ছেসূচি পেজে যোগ করা হয়নি।',
                'move-to-cart' => 'কার্টে সরিয়ে যান',
                'moved' => 'আইটেমটি সফলভাবে কার্টে সরানো হয়েছে',
                'moved-success' => 'আইটেমটি সফলভাবে কার্টে সরানো হয়েছে',
                'page-title' => 'ইচ্ছেসূচি',
                'product-removed' => 'প্রশাসন দ্বারা সরানো হয়েছে সৌজন্যে প্রোডাক্টটি আর উপলব্ধ নয়',
                'profile' => 'প্রোফাইল',
                'remove' => 'সরান',
                'remove-all-success' => 'আপনার ইচ্ছেসূচি থেকে সমস্ত আইটেম সরানো হয়েছে',
                'remove-fail' => 'আইটেম ইচ্ছেসূচি থেকে সরানো সম্ভব হয়নি',
                'removed' => 'আইটেম সফলভাবে ইচ্ছেসূচি থেকে সরানো হয়েছে',
                'see-details' => 'বিশদ দেখুন',
                'success' => 'আইটেম সফলভাবে ইচ্ছা সূচিতে যুক্ত হয়েছে',
                'title' => 'ইচ্ছেসূচি',
            ],

            'gdpr' => [
                'create-success' => 'অনুরোধ সফলভাবে তৈরি হয়েছে',
                'revoked-successfully' => 'অনুরোধ সফলভাবে বাতিল হয়েছে',
                'success-verify' => 'সফলতা! যাচাইকরণ ইমেল পাঠানো হয়েছে।',
                'success-verify-email-unsent' => 'সফলতা! যাচাইকরণ ইমেল পাঠানো হয়নি।',
                'unable-to-sent' => 'ইমেল পাঠাতে অক্ষম।',

                'index' => [
                    'create-btn' => 'অনুরোধ তৈরি করুন',
                    'html' => 'HTML',
                    'pdf' => 'PDF',
                    'title' => 'জিডিপিআর ডেটা অনুরোধ',

                    'modal' => [
                        'message' => 'বার্তা',
                        'save' => 'সংরক্ষণ করুন',
                        'title' => 'নতুন অনুরোধ তৈরি করুন',

                        'type' => [
                            'choose' => 'নির্বাচন করুন',
                            'delete' => 'মুছুন',
                            'title' => 'ধরন',
                            'update' => 'আপডেট',
                        ],
                    ],

                    'datagrid' => [
                        'completed' => 'সম্পন্ন',
                        'date' => 'তারিখ',
                        'declined' => 'প্রত্যাখ্যাত',
                        'delete' => 'মুছে ফেলা',
                        'id' => 'আইডি',
                        'message' => 'বার্তা',
                        'pending' => 'মুলতুবি',
                        'processing' => 'প্রক্রিয়াধীন',
                        'revoke-btn' => 'বাতিল করুন',
                        'revoked' => 'বাতিল',
                        'status' => 'অবস্থা',
                        'type' => 'ধরন',
                        'update' => 'আপডেট',
                    ],
                ],

                'pdf' => [
                    'title' => 'ডিফল্ট স্টোর ভিউ',

                    'account-info' => [
                        'dob' => 'জন্ম তারিখ',
                        'email' => 'ইমেল',
                        'first-name' => 'প্রথম নাম',
                        'gender' => 'লিঙ্গ',
                        'last-name' => 'শেষ নাম',
                        'phone' => 'ফোন',
                        'title' => 'অ্যাকাউন্ট তথ্য',
                    ],

                    'address-info' => [
                        'address' => 'ঠিকানা',
                        'address1' => 'ঠিকানা ১',
                        'address2' => 'ঠিকানা ২',
                        'city' => 'শহর',
                        'company' => 'কোম্পানি',
                        'country' => 'দেশ',
                        'first-name' => 'প্রথম নাম',
                        'last-name' => 'শেষ নাম',
                        'phone' => 'ফোন',
                        'postcode' => 'পোস্টকোড',
                        'state' => 'অবস্থা',
                        'title' => 'ঠিকানা তথ্য',
                        'vat-id' => 'ভ্যাট আইডি',
                    ],

                    'order-info' => [
                        'amount' => 'পরিমাণ',
                        'order-id' => 'অর্ডার আইডি',
                        'product-name' => 'পণ্যের নাম',
                        'qty' => 'পরিমাণ',
                        'shipping' => 'শিপিং',
                        'sku' => 'এসকেইউ',
                        'status' => 'অবস্থা',
                        'title' => 'অর্ডার তথ্য',
                        'type' => 'ধরন',
                    ],
                ],
            ],

            'rma' => [
                'index' => [
                    'title' => 'RMA',
                    'action' => 'অ্যাকশন',

                    'datagrid' => [
                        'id' => 'RMA আইডি',
                        'order-ref' => 'অর্ডার রেফারেন্স',
                        'rma-status' => 'অনুরোধের অবস্থা',
                        'create' => 'তৈরি হয়েছে',
                        'quantity' => 'পরিমাণ',
                    ],
                ],

                'create' => [
                    'action' => 'অ্যাকশন',
                    'sku' => 'SKU',
                    'price' => 'মূল্য',
                    'current-order-quantity' => 'বর্তমান অর্ডার পরিমাণ',
                    'product-already-raw' => 'এই পণ্যের সব আইটেম ইতিমধ্যেই রিটার্ন/বাতিলের জন্য অনুরোধ করা হয়েছে',
                    'resolution-type' => 'সমাধানের ধরন',
                    'select' => 'নির্বাচন করুন',
                    'return' => 'রিটার্ন/ফেরত',
                    'cancel-items' => 'আইটেম বাতিল করুন',
                    'product-delivery-status' => 'ডেলিভারির অবস্থা',
                    'package-condition' => 'প্যাকেজের অবস্থা',
                    'open' => 'খোলা',
                    'packed' => 'প্যাক করা',
                    'return-pickup-address' => 'রিটার্ন পিকআপ ঠিকানা',
                    'return-pickup-time' => 'রিটার্ন পিকআপ সময়',
                    'morning' => 'সকাল (১০টা - ১টা)',
                    'afternoon' => 'দুপুর (২টা - ৫টা)',
                    'evening' => 'সন্ধ্যা (৬টা - ৯টা)',
                    'images' => 'ছবি',
                ],

                'view' => [
                    'id' => 'RMA আইডি',
                    'enter-message' => 'এখানে আপনার বার্তা লিখুন',
                    'add-attachments' => 'সংযুক্তি যোগ করুন',
                    'send-message-btn' => 'বার্তা পাঠান',
                    'attachment' => 'সংযুক্তি',
                    'download' => 'ডাউনলোড',
                    'allowed-file-types' => 'অনুমোদিত ফাইলের ধরন',
                    'remove' => 'মুছুন',
                ],

                'terms' => [
                    'terms' => 'আমি শর্তাবলীতে সম্মত',
                    'read' => 'শর্তাবলী পড়ুন',
                ],
            ],
        ],
    ],

    'components' => [
        'accordion' => [
            'default-content' => 'ডিফল্ট কন্টেন্ট',
            'default-header' => 'ডিফল্ট হেডার',
        ],

        'drawer' => [
            'default-toggle' => 'ডিফল্ট টগল',
        ],

        'media' => [
            'index' => [
                'add-attachments' => 'সংযুক্তি যোগ করুন',
                'add-image' => 'ছবি / ভিডিও যোগ করুন',
                'invalid-file-type' => 'নির্বাচিত ফাইলের ধরন অনুমোদিত নয়।',
            ],
        ],

        'layouts' => [
            'header' => [
                'desktop' => [
                    'top' => [
                        'default-locale' => 'ডিফল্ট ভাষা',
                    ],

                    'bottom' => [
                        'all' => 'সব',
                        'back-button' => 'প্রধান মেনুতে ফিরে যান',
                        'bagisto' => 'বাগিস্টো',
                        'categories' => 'বিভাগ',
                        'compare' => 'তুলনা করুন',
                        'dropdown-text' => 'কার্ট, অর্ডার ও উইশলিস্ট পরিচালনা করুন',
                        'logout' => 'লগআউট',
                        'orders' => 'অর্ডার',
                        'profile' => 'প্রোফাইল',
                        'search' => 'অনুসন্ধান',
                        'search-text' => 'এখানে পণ্য অনুসন্ধান করুন',
                        'sign-in' => 'সাইন ইন',
                        'sign-up' => 'সাইন আপ',
                        'submit' => 'জমা দিন',
                        'welcome' => 'স্বাগতম',
                        'welcome-guest' => 'অতিথিকে স্বাগতম',
                        'wishlist' => 'উইশলিস্ট',
                    ],
                ],

                'mobile' => [
                    'account' => 'অ্যাকাউন্ট',
                    'back-button' => 'প্রধান মেনুতে ফিরে যান',
                    'bagisto' => 'বাগিস্টো',
                    'compare' => 'তুলনা করুন',
                    'currencies' => 'মুদ্রা',
                    'dropdown-text' => 'কার্ট, অর্ডার ও উইশলিস্ট পরিচালনা করুন',
                    'locales' => 'ভাষা',
                    'login' => 'সাইন আপ অথবা লগইন করুন',
                    'logout' => 'লগআউট',
                    'orders' => 'অর্ডার',
                    'profile' => 'প্রোফাইল',
                    'search' => 'অনুসন্ধান',
                    'search-text' => 'এখানে পণ্য অনুসন্ধান করুন',
                    'sign-in' => 'সাইন ইন',
                    'sign-up' => 'সাইন আপ',
                    'welcome' => 'স্বাগতম',
                    'welcome-guest' => 'অতিথিকে স্বাগতম',
                    'wishlist' => 'উইশলিস্ট',
                ],
            ],

            'webmcp' => [
                'add-to-wishlist' => 'নাম দিয়ে গ্রাহকের ইচ্ছেতালিকায় একটি পণ্য যোগ করুন।',
                'add-to-wishlist-query' => 'ইচ্ছেতালিকায় যোগ করার জন্য পণ্যের নাম বা URL কী।',
                'proceed-to-checkout' => 'কার্টে থাকা পণ্যগুলির জন্য অর্ডার দেওয়া শুরু করতে চেকআউট পৃষ্ঠাটি খুলুন।',
                'search-products' => 'কীওয়ার্ড দিয়ে স্টোর ক্যাটালগে পণ্য অনুসন্ধান করুন এবং মিলে যাওয়া ফলাফল খুলুন।',
                'search-products-query' => 'ক্যাটালগে অনুসন্ধান করার জন্য কীওয়ার্ড বা পণ্যের নাম।',
                'subscribe-newsletter' => 'স্টোরের নিউজলেটারে একটি ইমেল ঠিকানা সাবস্ক্রাইব করুন।',
                'subscribe-newsletter-email' => 'নিউজলেটারে সাবস্ক্রাইব করার জন্য ইমেল ঠিকানা।',
                'view-cart' => 'গ্রাহক যে পণ্যগুলি যোগ করেছেন তা পর্যালোচনা করতে শপিং কার্ট পৃষ্ঠাটি খুলুন।',
                'view-product' => 'পণ্যের নাম দিয়ে একটি পণ্যের বিস্তারিত পৃষ্ঠা খুলুন।',
                'view-product-query' => 'খোলার জন্য পণ্যের নাম বা URL কী।',
                'view-wishlist' => 'গ্রাহকের সংরক্ষিত পণ্যগুলি পর্যালোচনা করতে ইচ্ছেতালিকা পৃষ্ঠাটি খুলুন।',
            ],

            'footer' => [
                'about-us' => 'আমাদের সম্পর্কে',
                'contact-us' => 'যোগাযোগ করুন',
                'currency' => 'মুদ্রা',
                'customer-service' => 'গ্রাহক সেবা',
                'email' => 'ইমেইল',
                'footer-content' => 'ফুটারের সামগ্রী',
                'footer-text' => '© কপিরাইট 2010 - :current_year, Rancon LubMart। সমস্ত অধিকার সংরক্ষিত।',
                'locale' => 'লোকেল',
                'newsletter-text' => 'আমাদের মজাদার নিউজলেটারের জন্য প্রস্তুত থাকুন!',
                'order-return' => 'অর্ডার এবং রিটার্ন',
                'payment-policy' => 'পেমেন্ট নীতি',
                'privacy-cookies-policy' => 'গোপনীয়তা এবং কুকিজ নীতি',
                'shipping-policy' => 'শিপিং নীতি',
                'subscribe' => 'সাবস্ক্রাইব',
                'subscribe-newsletter' => 'নিউজলেটার সাবস্ক্রাইব করুন',
                'subscribe-stay-touch' => 'যোগাযোগে থাকতে সাবস্ক্রাইব করুন।',
                'whats-new' => 'নতুন কি',
            ],

            'cookie' => [
                'index' => [
                    'privacy-policy' => 'গোপনীয়তা নীতি',
                    'reject' => 'প্রত্যাখ্যান করুন',
                    'accept' => 'গ্রহণ করুন',
                    'learn-more-and-customize' => 'আরও জানুন এবং কাস্টমাইজ করুন',
                ],

                'consent' => [
                    'your-cookie-consent-preferences' => 'আপনার কুকি সম্মতি পছন্দগুলি',
                    'save-and-continue' => 'সংরক্ষণ করুন এবং চালিয়ে যান',
                    'strictly-necessary' => 'কঠোরভাবে প্রয়োজনীয়',
                    'basic-interactions' => 'মৌলিক ইন্টারঅ্যাকশন এবং কার্যকারিতা',
                    'experience-enhancement' => 'অভিজ্ঞতা উন্নতি',
                    'measurements' => 'পরিমাপ',
                    'targeting-and-advertising' => 'টার্গেটিং এবং বিজ্ঞাপন',
                ],
            ],
        ],

        'datagrid' => [
            'toolbar' => [
                'length-of' => ':দৈর্ঘ্য',
                'results' => ':মোট ফলাফল',
                'selected' => ':নির্বাচিত ফলাফল',

                'mass-actions' => [
                    'must-select-a-mass-action' => 'আপনাকে একটি সমমূল্য কর্ম নির্বাচন করতে হবে।',
                    'must-select-a-mass-action-option' => 'আপনাকে একটি সমমূল্য কর্ম বিকল্প নির্বাচন করতে হবে।',
                    'no-records-selected' => 'কোনও রেকর্ড নির্বাচিত হয়নি।',
                    'select-action' => 'কর্ম নির্বাচন করুন',
                ],

                'search' => [
                    'title' => 'অনুসন্ধান',
                ],

                'filter' => [
                    'apply-filter' => 'ফিল্টার প্রয়োগ করুন',
                    'title' => 'ফিল্টার',

                    'dropdown' => [
                        'select' => 'নির্বাচন করুন',

                        'searchable' => [
                            'at-least-two-chars' => 'কমপক্ষে 2 টি অক্ষর লিখুন...',
                            'no-results' => 'কোনও ফলাফল পাওয়া যায়নি...',
                        ],
                    ],

                    'custom-filters' => [
                        'clear-all' => 'সব মুছুন',
                    ],

                    'number-options' => [
                        'between' => 'Between',
                        'equals' => 'Equals',
                        'from' => 'From',
                        'greater-than' => 'Greater Than',
                        'greater-than-or-equal' => 'Greater Than Or Equal',
                        'less-than' => 'Less Than',
                        'less-than-or-equal' => 'Less Than Or Equal',
                        'to' => 'To',
                    ],
                ],
            ],

            'table' => [
                'actions' => 'কর্ম',
                'next-page' => 'পরবর্তী পৃষ্ঠা',
                'no-records-available' => 'কোনও রেকর্ড পাওয়া যায়নি।',
                'of' => 'মোট :মোট আইটেমের মধ্যে',
                'page-navigation' => 'পৃষ্ঠা নেভিগেশন',
                'page-number' => 'পৃষ্ঠা নম্বর',
                'previous-page' => 'পূর্ববর্তী পৃষ্ঠা',
                'showing' => 'দেখাচ্ছে :firstItem',
                'to' => 'পর্যন্ত :lastItem',
            ],
        ],

        'modal' => [
            'default-content' => 'ডিফল্ট কন্টেন্ট',
            'default-header' => 'ডিফল্ট হেডার',

            'confirm' => [
                'agree-btn' => 'সম্মত',
                'disagree-btn' => 'অসম্মত',
                'message' => 'আপনি কি নিশ্চিত যে আপনি এই কাজটি সম্পাদন করতে চান?',
                'title' => 'আপনি কি নিশ্চিত?',
            ],
        ],

        'products' => [
            'card' => [
                'add-to-cart' => 'কার্টে যোগ করুন',
                'add-to-compare' => 'তুলনা তালিকায় যোগ করুন',
                'add-to-compare-success' => 'আইটেমটি তৈরি হয়েছে তুলনা তালিকায় সফলভাবে যোগ করা হয়েছে।',
                'add-to-wishlist' => 'ইচ্ছেতালিকা যোগ করুন',
                'already-in-compare' => 'আইটেমটি ইতিমধ্যে তুলনা তালিকায় যোগ করা হয়েছে।',
                'new' => 'নতুন',
                'review-description' => 'এই পণ্যটি প্রথম পর্যালোচনা করুন',
                'sale' => 'বিক্রয়',
            ],

            'carousel' => [
                'next' => 'পরবর্তী',
                'previous' => 'পূর্ববর্তী',
                'view-all' => 'সব দেখুন',
            ],

            'ratings' => [
                'title' => 'রেটিংস',
            ],
        ],

        'story' => [
            'figures' => [
                'grades' => 'সান্দ্রতা গ্রেড',
                'products' => 'পণ্য',
                'title' => 'আমাদের সংগ্রহ',
                'vehicles' => 'যানবাহনের ধরন',
            ],

            'protection' => [
                'kept' => 'সুরক্ষিত',
                'text' => 'ধাতুর মাঝে তেলের স্তর না থাকলে বিয়ারিং পৃষ্ঠ ক্ষয়ে যায়। স্তর রাখুন, যন্ত্রাংশ টিকবে।',
                'title' => 'যেখানে তেল নেই সেখানেই ক্ষয়',
                'worn' => 'সুরক্ষাহীন',
            ],

            'viscosity' => [
                'cold' => 'ঠান্ডা',
                'hot' => 'গরম',
                'text' => 'প্রথম সংখ্যাটি ঠান্ডায় প্রবাহ, দ্বিতীয়টি গরমে স্থায়িত্ব। মাল্টিগ্রেড দুটোই করে।',
                'title' => 'আপনার গ্রেড বাছুন',
            ],

            'engine' => [
                'eyebrow' => 'ইঞ্জিনের ভিতরে',
                'step-gallery' => 'গ্যালারি বেয়ে ওঠে',
                'step-ring' => 'বোরে আস্তরণ দেয়',
                'step-sump' => 'সাম্প ভরে',
                'text' => 'আসল শেল লুব্রিক্যান্ট সাম্প থেকে শীর্ষ রিং পর্যন্ত প্রতিটি চলমান অংশে পৌঁছায়।',
                'title' => 'সম্পূর্ণ সুরক্ষা',
            ],
        ],

        'categories' => [
            'scroller' => [
                'scroll-hint' => 'দেখতে স্ক্রোল করুন',
                'view-range' => 'রেঞ্জ দেখুন',
            ],
        ],

        'range-slider' => [
            'max-range' => 'সর্বাধিক পরিসীমা',
            'min-range' => 'ন্যূনতম পরিসীমা',
            'range' => 'পরিসীমা:',
        ],

        'carousel' => [
            'explore-range' => 'রেঞ্জ দেখুন',
            'go-to-slide' => 'স্লাইড :number-এ যান',
            'image-slide' => 'চিত্র স্লাইড',
            'next' => 'পরবর্তী',
            'previous' => 'পূর্ববর্তী',
        ],

        'quantity-changer' => [
            'decrease-quantity' => 'পরিমাণ হ্রাস',
            'increase-quantity' => 'পরিমাণ বৃদ্ধি',
            'remove-item' => 'আইটেম সরান',
        ],
    ],

    'products' => [
        'catalog' => [
            'all-products' => 'All Products',
            'browse-by-category' => 'Browse by Category',
            'browse-by-spec' => 'Browse by Specification',
            'empty' => 'No products match these filters.',
            'heading' => 'The Complete Catalogue',
            'meta-description' => 'Browse the complete product catalogue by category, brand and specification.',
            'showing' => 'Showing :count of :total products',
            'subtitle' => 'Everything we stock, in one place. Browse by category, by brand, or by the specification you need.',
            'title' => 'All Products',
        ],
        'prices' => [
            'grouped' => [
                'starting-at' => 'শুরু হচ্ছে',
            ],

            'configurable' => [
                'as-low-as' => 'যেহেতু সর্বনিম্ন',
            ],
        ],

        'sort-by' => [
            'title' => 'দ্বারা বাছাই করুন',
        ],

        'view' => [
            'type' => [
                'simple' => [
                    'customizable-options' => [
                        'none' => 'কিছু নেই',
                        'total-amount' => 'মোট পরিমাণ',
                    ],
                ],

                'configurable' => [
                    'select-options' => 'দয়া করে একটি বিকল্প নির্বাচন করুন',
                    'select-above-options' => 'দয়া করে উপরের বিকল্পগুলি নির্বাচন করুন',
                ],

                'bundle' => [
                    'none' => 'কোনটি নেই',
                    'total-amount' => 'মোট পরিমাণ',
                ],

                'downloadable' => [
                    'links' => 'লিংকগুলি',
                    'sample' => 'নমুনা',
                    'samples' => 'নমুনা',
                ],

                'grouped' => [
                    'name' => 'নাম',
                ],

                'booking' => [
                    'location' => 'অবস্থান',
                    'view-on-map' => 'মানচিত্রে দেখুন',

                    'cancellation-not-allowed' => [
                        'title' => 'বাতিলকরণ অনুমোদিত নয়',
                        'description' => 'একবার এই বুকিং করা হয়ে গেলে, এটি বাতিল করা যাবে না। চেকআউটের আগে দয়া করে বিবরণ সাবধানে পর্যালোচনা করুন।',
                    ],

                    'default' => [
                        'slot-duration-in-minutes' => ':minutes মিনিট',
                        'slot-duration' => 'স্লট সময়কাল',
                    ],

                    'appointment' => [
                        'closed' => 'বন্ধ',
                        'see-details' => 'বিস্তারিত দেখুন',
                        'slot-duration' => 'স্লট সময়কাল',
                        'slot-duration-in-minutes' => ':minutes মিনিট',
                        'today-availability' => 'আজকের প্রাপ্যতা',
                    ],

                    'event' => [
                        'book-your-ticket' => 'আপনার টিকিট বুক করুন',
                        'title' => 'ইভেন্ট :',
                        'total-price' => 'মোট মূল্য',
                        'summary-title' => 'আপনার বুকিং',
                        'ticket-line' => ':name (:count × :price)',
                        'base-fee-line' => 'মৌলিক ফি (:count × :price)',
                        'ticket' => 'টিকিট',
                        'tickets' => 'টিকিট',
                        'total' => 'মোট',
                        'select-tickets-hint' => 'মোট দেখতে আপনার টিকিটের সংখ্যা নির্বাচন করুন।',
                        'starting-from' => 'শুরু হচ্ছে',
                        'base-fee-note' => 'মৌলিক ফি + সবচেয়ে সস্তা টিকিটের দাম অন্তর্ভুক্ত। আপনার টিকিট নির্বাচন অনুযায়ী মোট আপডেট হয়।',
                    ],

                    'rental' => [
                        'choose-rent-option' => 'ভাড়া বিকল্প নির্বাচন করুন',
                        'daily-basis' => 'দৈনিক ভিত্তিতে',
                        'from' => 'থেকে',
                        'hourly-basis' => 'ঘণ্টা ভিত্তিতে',
                        'rent-an-item' => 'একটি আইটেম ভাড়া করুন',
                        'select-date' => 'তারিখ নির্বাচন করুন',
                        'select-rent-time' => 'ভাড়া সময় নির্বাচন করুন',
                        'select-slot' => 'স্লট নির্বাচন করুন',
                        'select-time-slot' => 'সময় স্লট নির্বাচন করুন',
                        'slot' => 'স্লট',
                        'no-slots-available' => 'কোন স্লট উপলব্ধ নেই',
                        'to' => 'পর্যন্ত',
                        'summary-title' => 'আপনার ভাড়া',
                        'base-rental-fee' => 'মৌলিক ভাড়ার ফি',
                        'daily-rate-line' => 'দৈনিক হার (:count × :rate)',
                        'hourly-rate-line' => 'প্রতি ঘণ্টার হার (:count × :rate)',
                        'day' => 'দিন',
                        'days' => 'দিন',
                        'hour' => 'ঘণ্টা',
                        'hours' => 'ঘণ্টা',
                        'total' => 'মোট',
                        'select-dates-hint' => 'মোট দেখতে ভাড়ার তারিখ নির্বাচন করুন।',
                        'starting-from' => 'শুরু হচ্ছে',
                        'rental-fee-note' => 'মৌলিক ফি + সর্বনিম্ন ভাড়ার হার অন্তর্ভুক্ত। আপনার নির্বাচিত সময়কাল অনুযায়ী মোট আপডেট হয়।',
                    ],

                    'table' => [
                        'book-a-table' => 'একটি টেবিল বুক করুন',
                        'closed' => 'বন্ধ',
                        'slot-duration' => 'স্লট সময়কাল',
                        'slot-duration-in-minutes' => ':minutes মিনিট',
                        'slots-for-all-days' => 'সব দিনের জন্য দেখান',
                        'special-notes' => 'বিশেষ অনুরোধ/নোট',
                        'today-availability' => 'আজকের প্রাপ্যতা',
                    ],

                    'slots' => [
                        'book-an-appointment' => 'একটি অ্যাপয়েন্টমেন্ট বুক করুন',
                        'date' => 'তারিখ',
                        'no-slots-available' => 'কোন স্লট উপলব্ধ নেই',
                        'select-slot' => 'স্লট নির্বাচন করুন',
                        'title' => 'স্লট',
                    ],
                ],
            ],

            'gallery' => [
                'product-image' => 'পণ্য ইমেজ',
                'thumbnail-image' => 'থাম্বনেল ইমেজ',
            ],

            'reviews' => [
                'attachments' => 'সংযুক্তিসমূহ',
                'cancel' => 'বাতিল করুন',
                'comment' => 'মন্তব্য',
                'customer-review' => 'গ্রাহক পর্যালোচনা',
                'empty-review' => 'কোনও রিভিউ পাওয়া যায়নি, প্রথমে এই পণ্যটির প্রতি রিভিউ দিতে হয়',
                'failed-to-upload' => 'চিত্র আপলোড ব্যর্থ হয়েছে',
                'load-more' => 'আরও লোড করুন',
                'name' => 'নাম',
                'rating' => 'রেটিং',
                'ratings' => 'রেটিং',
                'submit-review' => 'রিভিউ জমা দিন',
                'success' => 'রিভিউ সফলভাবে দাখিল করা হয়েছে।',
                'title' => 'শিরোনাম',
                'translate' => 'অনুবাদ',
                'translating' => 'অনুবাদ হচ্ছে...',
                'write-a-review' => 'রিভিউ লিখুন',
            ],

            'contact-us' => [
                'about' => 'আমাদের একটি নোট লিখুন এবং আমরা সম্ভাব্যতঃ সবচেয়ে দ্রুত আপনাকে ফিরে যাব',
                'desc' => 'আপনার মনে কি আছে?',
                'describe-here' => 'এখানে বর্ণনা করুন',
                'email' => 'ইমেইল',
                'message' => 'বার্তা',
                'name' => 'নাম',
                'phone-number' => 'ফোন নম্বর',
                'submit' => 'জমা দিন',
                'title' => 'যোগাযোগ করুন',
            ],

            'add-to-cart' => 'কার্টে যোগ করুন',
            'add-to-compare' => 'পণ্য তুলনা তালিকায় যোগ করা হয়েছে।',
            'add-to-wishlist' => 'ইচ্ছেতালিকা যোগ করুন',
            'additional-information' => 'অতিরিক্ত তথ্য',
            'already-in-compare' => 'পণ্যটি ইতিমধ্যে তুলনা তালিকায় যোগ করা হয়েছে।',
            'buy-now' => 'এখন ক্রয় করুন',
            'compare' => 'তুলনা',
            'description' => 'বিবরণ',
            'related-product-title' => 'সাথে সম্পর্কিত পণ্যগুলি',
            'review' => 'রিভিউ',
            'tax-inclusive' => 'সমস্ত কর সহ',
            'up-sell-title' => 'আমরা অন্যান্য পণ্য পেয়েছি যা আপনার পছন্দের হতে পারে!',
        ],

        'type' => [
            'abstract' => [
                'offers' => 'প্রতি একক মূল্যে কিনুন :qty এবং :discount সেভ করুন',
            ],
        ],

        'booking' => [
            'closed' => 'বন্ধ',

            'cart' => [
                'booking-from' => 'বুকিং থেকে',
                'booking-location' => 'অবস্থান',
                'booking-till' => 'বুকিং পর্যন্ত',
                'charged-per' => 'প্রতি চার্জ',
                'daily' => 'দৈনিক ভিত্তিতে',
                'event-from' => 'ইভেন্ট থেকে',
                'event-location' => 'ইভেন্টের অবস্থান',
                'event-ticket' => 'ইভেন্ট টিকিট',
                'event-till' => 'ইভেন্ট পর্যন্ত',
                'guest-limit' => 'প্রতি টেবিলে অতিথি সীমা',
                'hourly' => 'ঘণ্টা ভিত্তিতে',

                'integrity' => [
                    'event' => [
                        'expired' => 'এই ইভেন্টটির মেয়াদ শেষ হয়ে গেছে।',
                        'exceeds_available' => 'এই ইভেন্টের জন্য মাত্র :qty টিকেট উপলব্ধ।',
                        'sold_out' => 'এই ইভেন্টের টিকেট বিক্রি শেষ হয়ে গেছে।',
                        'ticket_exceeds_available' => 'শুধুমাত্র :qty ":ticket" টিকেট উপলব্ধ।',
                        'ticket_sold_out' => '":ticket" টিকেট বিক্রি শেষ।',
                    ],

                    'missing_options' => 'এই পণ্যের জন্য বিকল্পগুলি অনুপস্থিত।',
                    'inventory_warning' => 'অনুরোধকৃত পরিমাণ উপলব্ধ নেই, দয়া করে পরে আবার চেষ্টা করুন।',
                    'rental_unavailable' => 'এই ভাড়া পণ্যটি নির্বাচিত তারিখগুলির জন্য উপলব্ধ নেই।',
                    'select_hourly_duration' => 'কমপক্ষে এক ঘণ্টার সময়কাল নির্বাচন করুন।',
                ],

                'per-guest' => 'প্রতি অতিথি',
                'per-table' => 'প্রতি টেবিল',
                'rent-from' => 'ভাড়া থেকে',
                'rent-till' => 'ভাড়া পর্যন্ত',
                'rent-type' => 'ভাড়া ধরন',
                'renting_type' => 'ভাড়া ধরন',
                'special-note' => 'বিশেষ অনুরোধ/নোট',
            ],

            'per-ticket-price' => ':price প্রতি টিকিট',
        ],
    ],

    'categories' => [
        'filters' => [
            'clear-all' => 'সব মুছুন',
            'filter' => 'ফিল্টার',
            'filters' => 'ফিল্টার:',

            'search' => [
                'load-more' => 'আরও লোড করুন',
                'loading' => 'লোড হচ্ছে...',
                'no-options-available' => 'কোনো বিকল্প উপলব্ধ নেই।',
                'results-info' => ':totalCount বিকল্পের মধ্যে :currentCount দেখানো হচ্ছে',
                'title' => 'অনুসন্ধান',
            ],

            'sort' => 'সাজান',
        ],

        'toolbar' => [
            'grid' => 'Cuadrícula',
            'list' => 'Lista',
            'show' => 'Mostrar',
        ],

        'view' => [
            'empty' => 'এই বিভাগে কোনও পণ্য উপলব্ধ নেই',
            'load-more' => 'আরও দেখুন',
        ],
    ],

    'search' => [
        'title' => 'এগুলি :query এর জন্য ফলাফল',
        'suggest' => 'এর পরিবর্তে অনুসন্ধান করুন',
        'results' => 'অনুসন্ধান ফলাফল',

        'images' => [
            'index' => [
                'only-images-allowed' => 'শুধুমাত্র চিত্র (.jpeg, .jpg, .png, ..) অনুমোদিত।',
                'search' => 'খুঁজুন',
                'size-limit-error' => 'সাইজ সীমা পার',
                'something-went-wrong' => 'কিছু ভুল হয়েছে, দয়া করে পরে আবার চেষ্টা করুন।',
            ],

            'results' => [
                'analyzed-keywords' => 'বিশ্লেষণ করা শব্দ:',
            ],
        ],
    ],

    'compare' => [
        'already-added' => 'আইটেম ইতিমধ্যে তুলনা তালিকায় যোগ করা হয়েছে',
        'delete-all' => 'সব মুছুন',
        'empty-text' => 'আপনার তুলনা তালিকায় আপনি কোনও আইটেম নেই',
        'item-add-success' => 'আইটেম সফলভাবে তুলনা তালিকায় যোগ করা হয়েছে',
        'product-compare' => 'পণ্য তুলনা',
        'remove-all-success' => 'সমস্ত আইটেম সফলভাবে সরানো হয়েছে',
        'remove-error' => 'কিছু ভুল হয়েছে, দয়া করে পরে আবার চেষ্টা করুন।',
        'remove-success' => 'আইটেম সফলভাবে সরানো হয়েছে',
        'title' => 'পণ্য তুলনা',
    ],

    'checkout' => [
        'success' => [
            'info' => 'আমরা আপনাকে আপনার অর্ডারের বিশদ এবং ট্র্যাকিং তথ্য ইমেল করব',
            'order-id-info' => 'আপনার অর্ডার আইডি হলো #:order_id',
            'thanks' => 'আপনার অর্ডারের জন্য ধন্যবাদ!',
            'title' => 'সফলভাবে অর্ডার স্থাপন',
        ],

        'cart' => [
            'continue-to-checkout' => 'চেকআউট চালিয়ে যান',
            'free-gift' => [
                'added' => 'Your free gift has been added to the cart.',
                'choose-size' => 'Choose a size',
                'claim' => 'Claim Gift',
                'earned' => 'You have earned a free gift!',
                'not-available' => 'This gift is no longer available for your cart.',
            ],
            'illegal' => 'পরিমাণ একের চেয়ে কম হতে পারে না।',
            'inactive' => 'আইটেমটি নিষ্ক্রিয় হয়ে গিয়েছে এবং পরবর্তীকার কার্ট থেকে সরানো হয়েছে।',
            'inactive-add' => 'নিষ্ক্রিয় আইটেমটি কার্টে যোগ করা যাবে না।',
            'inventory-warning' => 'অনুরোধকৃত পরিমাণ উপলব্ধ নেই, দয়া করে পরে আবার চেষ্টা করুন।',
            'item-add-to-cart' => 'পণ্য সফলভাবে যোগ করা হয়েছে',
            'minimum-order-message' => 'সর্বনিম্ন অর্ডার পরিমাণ হলো',
            'missing-fields' => 'এই পণ্যের জন্য কিছু প্রয়োজনীয় ক্ষেত্র অনুপস্থিত।',
            'missing-options' => 'এই পণ্যের জন্য বিকল্প অনুপস্থিত।',
            'paypal-payment-cancelled' => 'পেপ্যাল পেমেন্ট বাতিল করা হয়েছে.',
            'qty-missing' => 'কমপক্ষে একটি পণ্য করে একটি পরিমাণ থাকতে হবে।',
            'return-to-shop' => 'দোকানে ফিরে যান',
            'rule-applied' => 'কার্ট নিয়ম প্রয়োগ হয়েছে',
            'select-hourly-duration' => 'এক ঘণ্টা পর্যায়ের একটি স্লট নির্বাচন করুন।',
            'specify-payment-method' => 'অনুগ্রহ করে একটি পেমেন্ট পদ্ধতি নির্দিষ্ট করুন।',
            'specify-shipping-method' => 'অনুগ্রহ করে একটি শিপিং পদ্ধতি নির্দিষ্ট করুন।',
            'success-remove' => 'পণ্য সফলভাবে কার্ট থেকে সরানো হয়েছে।',
            'suspended-account-message' => 'আপনার অ্যাকাউন্ট স্থগিত করা হয়েছে',

            'index' => [
                'bagisto' => 'বাগিস্তো',
                'cart' => 'কার্ট',
                'continue-shopping' => 'কেনাকাটা চালিয়ে যান',
                'empty-product' => 'আপনার কার্টে কোনও পণ্য নেই।',
                'excl-tax' => 'কর বাদে:',
                'home' => 'হোম',
                'items-selected' => ':count টি আইটেম নির্বাচিত',
                'move-to-wishlist' => 'ইচ্ছেসূচি তালিকায় সরান',
                'move-to-wishlist-success' => 'নির্বাচিত আইটেমগুলি সফলভাবে ইচ্ছেসূচি তালিকায় সরানো হয়েছে।',
                'price' => 'মূল্য',
                'product-name' => 'পণ্যের নাম',
                'quantity' => 'পরিমাণ',
                'quantity-update' => 'পরিমাণ সফলভাবে আপডেট হয়েছে',
                'remove' => 'সরান',
                'remove-selected-success' => 'নির্বাচিত আইটেমগুলি সফলভাবে কার্ট থেকে সরানো হয়েছে।',
                'see-details' => 'বিশদ দেখুন',
                'select-all' => 'সব চেক করুন',
                'select-cart-item' => 'কার্ট আইটেম চেক করুন',
                'tax' => 'কর',
                'total' => 'মোট',
                'update-cart' => 'কার্ট আপডেট করুন',
                'view-cart' => 'কার্ট দেখুন',

                'cross-sell' => [
                    'title' => 'আরও বিকল্প',
                ],
            ],

            'mini-cart' => [
                'continue-to-checkout' => 'চেকআউট চালিয়ে যান',
                'empty-cart' => 'আপনার কার্ট খালি',
                'excl-tax' => 'බද. බද්ද:',
                'offer-on-orders' => 'আপনার 1 ম অর্ডারে 30% ছাড়',
                'remove' => 'সরান',
                'see-details' => 'বিশদ দেখুন',
                'shopping-cart' => 'শপিং কার্ট',
                'subtotal' => 'সাবটোটাল',
                'view-cart' => 'কার্ট দেখুন',
            ],

            'summary' => [
                'cart-summary' => 'কার্ট সংক্ষেপ',
                'delivery-charges' => 'ডেলিভারি চার্জ',
                'delivery-charges-excl-tax' => 'ডেলিভারি চার্জ (কর বাদে)',
                'delivery-charges-incl-tax' => 'ডেলিভারি চার্জ (কর সহ)',
                'discount-amount' => 'ছাড়ের পরিমাণ',
                'grand-total' => 'সর্বমোট',
                'included' => 'অন্তর্ভুক্ত',
                'excl-tax' => 'কর ব্যতীত:',
                'incl-tax' => 'ট্যাক্স সহ:',
                'items-discount' => 'আইটেম ছাড়',
                'shipping-discount' => 'শিপিং ছাড়',
                'place-order' => 'অর্ডার করুন',
                'proceed-to-checkout' => 'চেকআউটে এগিয়ে যান',
                'sub-total' => 'সাবটোটাল',
                'sub-total-excl-tax' => 'সাবটোটাল (কর বাদে)',
                'sub-total-incl-tax' => 'সাবটোটাল (কর সহ)',
                'tax' => 'কর',

                'estimate-shipping' => [
                    'country' => 'দেশ',
                    'info' => 'আপনার গন্তব্যের জন্য শিপিং এবং কর আনুমান পেতে দেশান্তর করুন।',
                    'postcode' => 'জিপ/পোস্টকোড',
                    'select-country' => 'দেশ নির্বাচন করুন',
                    'select-state' => 'রাষ্ট্র নির্বাচন করুন',
                    'state' => 'রাষ্ট্র',
                    'title' => 'শিপিং এবং কর আনুমান করুন',
                ],
            ],
        ],

        'onepage' => [
            'address' => [
                'add-new' => 'নতুন ঠিকানা যোগ করুন',
                'add-new-address' => 'নতুন ঠিকানা যোগ করুন',
                'back' => 'পৃষ্ঠায় ফিরুন',
                'billing-address' => 'বিলিং ঠিকানা',
                'check-billing-address' => 'বিলিং ঠিকানা অনুপস্থিত।',
                'check-shipping-address' => 'প্রেরণের ঠিকানা অনুপস্থিত।',
                'city' => 'শহর',
                'company-name' => 'কোম্পানির নাম',
                'confirm' => 'নিশ্চিত করুন',
                'country' => 'দেশ',
                'email' => 'ইমেইল',
                'first-name' => 'প্রথম নাম',
                'last-name' => 'শেষ নাম',
                'postcode' => 'জিপ/পোস্টকোড',
                'proceed' => 'এগিয়ে যান',
                'same-as-billing' => 'শিপিংের জন্য একই ঠিকানা ব্যবহার করবেন?',
                'save' => 'সংরক্ষণ করুন',
                'save-address' => 'এইটি ঠিকানা বইয়ে সংরক্ষণ করুন',
                'select-country' => 'দেশ নির্বাচন করুন',
                'select-state' => 'রাষ্ট্র নির্বাচন করুন',
                'shipping-address' => 'শিপিং ঠিকানা',
                'state' => 'রাষ্ট্র',
                'street-address' => 'রাস্তার ঠিকানা',
                'telephone' => 'টেলিফোন',
                'title' => 'ঠিকানা',
                'vat-id' => 'ভ্যাট আইডি',
            ],

            'index' => [
                'checkout' => 'চেকআউট',
                'home' => 'হোম',
            ],

            'payment' => [
                'payment-method' => 'পেমেন্ট মেথড',
            ],

            'shipping' => [
                'shipping-method' => 'শিপিং মেথড',
            ],

            'summary' => [
                'cart-summary' => 'কার্ট সংক্ষেপ',
                'delivery-charges' => 'ডেলিভারি চার্জ',
                'delivery-charges-excl-tax' => 'ডেলিভারি চার্জ (কর বাদে)',
                'delivery-charges-incl-tax' => 'ডেলিভারি চার্জ (কর সহ)',
                'discount-amount' => 'ছাড়ের পরিমাণ',
                'excl-tax' => 'কর বাদে:',
                'grand-total' => 'সর্বমোট',
                'included' => 'অন্তর্ভুক্ত',
                'incl-tax' => 'ট্যাক্স সহ:',
                'items-discount' => 'আইটেম ছাড়',
                'shipping-discount' => 'শিপিং ছাড়',
                'place-order' => 'অর্ডার স্থাপন',
                'price_and_qty' => ':price × :qty',
                'processing' => 'প্রক্রিয়াধীন',
                'sub-total' => 'সাবটোটাল',
                'sub-total-excl-tax' => 'সাবটোটাল (কর বাদে)',
                'sub-total-incl-tax' => 'সাবটোটাল (কর সহ)',
                'tax' => 'কর',

                'tax-included' => 'ট্যাক্স ইতিমধ্যে অন্তর্ভুক্ত:', ],
        ],

        'coupon' => [
            'already-applied' => 'কুপন কোড ইতিমধ্যে প্রয়োগ করা হয়েছে।',
            'applied' => 'কুপন প্রয়োগ হয়েছে',
            'apply' => 'কুপন প্রয়োগ করুন',
            'apply-issue' => 'কুপন কোড প্রয়োগ করা যায় না।',
            'button-title' => 'প্রয়োগ করুন',
            'code' => 'কুপন কোড',
            'discount' => 'কুপন ছাড়',
            'enter-your-code' => 'আপনার কোড লিখুন',
            'error' => 'কিছু ভুল হয়েছে',
            'invalid' => 'কুপন কোড অবৈধ।',
            'remove' => 'কুপন সরান',
            'subtotal' => 'সাবটোটাল',
            'success-apply' => 'কুপন কোড সফলভাবে প্রয়োগ হয়েছে।',
            'usage-limit-exceeded' => 'এই কুপন কোডটি তার ব্যবহারের সীমায় পৌঁছেছে এবং আর বৈধ নয়।',
        ],

        'login' => [
            'email' => 'ইমেইল',
            'password' => 'পাসওয়ার্ড',
            'title' => 'সাইন ইন',
        ],
    ],

    'home' => [
        'contact' => [
            'about' => 'আমাদের একটি নোট লিখুন এবং আমরা সম্ভাব্যতঃ সবচেয়ে দ্রুত আপনাকে ফিরে যাব',
            'desc' => ' আপনার মনে কি আছে?',
            'describe-here' => 'এখানে বর্ণনা করুন',
            'email' => 'ইমেইল',
            'message' => 'বার্তা',
            'name' => 'নাম',
            'phone-number' => 'ফোন নম্বর',
            'submit' => 'জমা দিন',
            'title' => 'যোগাযোগ করুন',
        ],

        'index' => [
            'brand-strip' => 'ব্র্যান্ড স্ট্রিপ',
            'categories-carousel' => 'ক্যাটাগরি ক্যারোসেল',
            'image-carousel' => 'ইমেজ ক্যারোসেল',
            'offer' => 'আপনার প্রথম অর্ডারে ৪০% পর্যন্ত ছাড় পান, এখনই কেনাকাটা করুন',
            'product-carousel' => 'প্রোডাক্ট ক্যারোসেল',
            'resend-verify-email' => 'যাচাইকরণ ইমেল পুনরায় পাঠান',
            'verify-email' => 'আপনার ইমেল অ্যাকাউন্ট যাচাই করুন',
        ],

        'thanks-for-contact' => 'আপনার মন্তব্য এবং প্রশ্নের জন্য আমাদের সাথে যোগাযোগ করার জন্য আপনাকে ধন্যবাদ। আমরা খুব শীঘ্রই আপনাকে উত্তর দেব।',
    ],

    'partials' => [
        'pagination' => [
            'next-page' => 'পরবর্তী পৃষ্ঠা',
            'pagination-showing' => ':firstItem থেকে :lastItem পর্যন্ত মোট :total নির্দেশিকা দেখানো হচ্ছে',
            'prev-page' => 'পূর্ববর্তী পৃষ্ঠা',
        ],
    ],

    'errors' => [
        'go-to-home' => 'হোমে যান',

        '404' => [
            'description' => 'ওহুপস! আপনি যে পৃষ্ঠাটি খুঁজছেন সেটি ছুটিতে আছে। দেখতে হয়েছে আমরা যা আপনি খোঁজছিলেন তা খুঁজে পেতে পারিনি।',
            'title' => '404 পেজ পাওয়া যায়নি',
        ],

        '401' => [
            'description' => 'ওহুপস! দেখা গিয়েছে আপনি এই পৃষ্ঠাতে অ্যাক্সেস করতে অনুমোদিত নেই। দেখতে হয়েছে আপনি প্রয়োজনীয় যে ভুমিকা প্রাপ্ত করছেন তা নেই।',
            'title' => '401 অনধিকৃত',
        ],

        '403' => [
            'description' => 'ওহুপস! এই পৃষ্ঠাটি নিষিদ্ধ। দেখা যায় আপনার এই বিষয়ে দেখতে নেওয়া যায় না। দেখতে হয়েছে আপনি এই বিষয়ে দেখার অনুমতি প্রাপ্ত নেই।',
            'title' => '403 নিষিদ্ধ',
        ],

        '500' => [
            'description' => 'ওহুপস! কিছু ভুল হয়ে গেছে। দেখা যায় আমরা সেই পৃষ্ঠাটি লোড করতে সমস্যা হয়েছে যেটি আপনি খুঁজছেন।',
            'title' => '500 আপেক্ষিক সার্ভার ত্রুটি',
        ],

        '503' => [
            'description' => 'ওহুপস! দেখা যায় আমরা সাময়িকভাবে নীচে আছি মেইন্টেন্যান্সের জন্য। দয়া করে কিছুটা পরে আবার চেক করুন।',
            'title' => '503 সার্ভিস অপ্রাপ্য',
        ],
    ],

    'layouts' => [
        'address' => 'ঠিকানা',
        'downloadable-products' => 'ডাউনলোড যোগ্য পণ্য',
        'gdpr-request' => 'জিডিপিআর অনুরোধ',
        'my-account' => 'আমার অ্যাকাউন্ট',
        'orders' => 'আদেশ',
        'profile' => 'প্রোফাইল',
        'reviews' => 'পর্যালোচনা',
        'rma' => 'RMA',
        'wishlist' => 'ইচ্ছাশূণ্যতা',
    ],

    'subscription' => [
        'already' => 'আপনি ইতিমধ্যে আমাদের সংবাদপত্রে সদস্য।',
        'subscribe-success' => 'আপনি সফলভাবে আমাদের সংবাদপত্রে সদস্য হন।',
        'unsubscribe-success' => 'আপনি সফলভাবে আমাদের সংবাদপত্র থেকে অনসাবস্য হন।',
    ],

    'emails' => [
        'dear' => 'প্রিয় :customer_name',
        'thanks' => 'আপনি যদি কোনও সাহায্য চান তবে দয়া করে আমাদের সাথে <a href=":link" style=":style">:email</a> যোগাযোগ করুন।<br/>ধন্যবাদ!',

        'customers' => [
            'registration' => [
                'credentials-description' => 'আপনার অ্যাকাউন্ট তৈরি হয়েছে। আপনার অ্যাকাউন্টের বিবরণ নীচে দেওয়া হলো:',
                'description' => 'আপনার অ্যাকাউন্ট সফলভাবে তৈরি হয়েছে এবং আপনি এখন আপনার ইমেল ঠিকানা এবং পাসওয়ার্ড ক্রেডেনশিয়াল ব্যবহার করে লগইন করতে পারেন। লগইন করার পরে, আপনি অন্যান্য সেবা অ্যাক্সেস করতে পারবেন যা পূর্বের আদেশগুলি পর্যালোচনা করা, ইচ্ছাশূণ্যতা দেখা এবং আপনার অ্যাকাউন্টের তথ্য সম্পাদনা করতে সক্ষম হবেন।',
                'greeting' => 'স্বাগতম এবং আমাদের সাথে নিবন্ধিত হওয়ার জন্য আপনাকে ধন্যবাদ!',
                'password' => 'পাসওয়ার্ড',
                'sign-in' => 'সাইন ইন করুন',
                'subject' => 'নতুন গ্রাহক নিবন্ধন',
                'username-email' => 'ব্যবহারকারীর নাম/ইমেল',
            ],

            'forgot-password' => [
                'description' => 'আপনি এই ইমেইলটি পেয়েছেন কারণ আমরা আপনার অ্যাকাউন্টের জন্য একটি পাসওয়ার্ড রিসেট অনুরোধ পেয়েছি।',
                'greeting' => 'পাসওয়ার্ড ভুলে গেছেন!',
                'reset-password' => 'পাসওয়ার্ড রিসেট',
                'subject' => 'পাসওয়ার্ড রিসেট ইমেইল',
            ],

            'update-password' => [
                'description' => 'আপনি এই ইমেইলটি পেয়েছেন কারণ আপনি আপনার পাসওয়ার্ড আপডেট করেছেন।',
                'greeting' => 'পাসওয়ার্ড আপডেট হয়েছে!',
                'subject' => 'পাসওয়ার্ড আপডেট করা হয়েছে',
            ],

            'verification' => [
                'description' => 'দয়া করে নীচের বাটনে ক্লিক করুন আপনার ইমেইল ঠিকানা যাচাই করতে।',
                'greeting' => 'স্বাগত!',
                'subject' => 'অ্যাকাউন্ট যাচাইকরণ ইমেইল',
                'verify-email' => 'ইমেইল ঠিকানা যাচাই করুন',
            ],

            'commented' => [
                'description' => 'নোট - :note',
                'subject' => 'নতুন মন্তব্য যোগ করা হয়েছে',
            ],

            'subscribed' => [
                'description' => 'অভিনন্দন এবং আমরা গর্বিত যে আপনি আমাদের সংবাদপত্র সম্প্রদানে আসার জন্য প্রস্তুত! আমরা আপনাকে সর্বশেষ সংবাদ, ট্রেন্ডস, এবং আপনার জন্য এক্সক্লুসিভ অফারে আপডেট করতে সত্বর অপেক্ষা করছি।',
                'greeting' => 'আমাদের সংবাদপত্রে স্বাগত!',
                'subject' => 'আপনি! আমাদের সংবাদপত্রে সদস্য হন',
                'unsubscribe' => 'সদস্যপত্র প্রত্যাহার করুন',
            ],

            'gdpr' => [
                'new-delete-request' => 'ডেটা মুছে ফেলার জন্য নতুন অনুরোধ',
                'new-update-request' => 'ডেটা আপডেট করার জন্য নতুন অনুরোধ',

                'new-request' => [
                    'delete-summary' => 'মুছে ফেলার অনুরোধের সংক্ষিপ্তসার',
                    'message' => 'বার্তা : ',
                    'request-status' => 'অনুরোধের অবস্থা : ',
                    'request-type' => 'অনুরোধের ধরন : ',
                    'update-summary' => 'আপডেট অনুরোধের সংক্ষিপ্তসার',
                ],

                'status-update' => [
                    'subject' => 'আপনার GDPR অনুরোধ আপডেট হয়েছে',
                    'summary' => 'আপনার GDPR অনুরোধের অবস্থা আপডেট হয়েছে',
                    'request-status' => 'অনুরোধের অবস্থা:',
                    'request-type' => 'অনুরোধের ধরন:',
                    'message' => 'বার্তা:',
                ],
            ],

            'reminder' => [
                'already-paid' => 'যদি আপনি ইতিমধ্যে অর্থপ্রদান করে থাকেন, তাহলে দয়া করে এই বার্তাটি উপেক্ষা করুন।',
                'invoice-overdue' => 'এটি একটি বন্ধুত্বপূর্ণ অনুস্মারক যে আপনার চালান এখন বকেয়া। অনুগ্রহ করে যত তাড়াতাড়ি সম্ভব অর্থপ্রদান করুন।',
                'subject' => 'চালান অনুস্মারক',
            ],
        ],

        'contact-us' => [
            'contact-from' => 'ওয়েবসাইট যোগাযোগ ফর্ম এর মাধ্যমে',
            'reply-to-mail' => 'এই ইমেইলে উত্তর দিন।',
            'reach-via-phone' => 'অথবা, আপনি ফোনে যোগাযোগ করতে পারেন',
            'inquiry-from' => 'প্রশ্নাবলী থেকে',
            'to' => 'যোগাযোগ করতে',
        ],

        'orders' => [
            'created' => [
                'greeting' => 'আপনার অর্ডার :order_id যেগুলি :created_at তারিখে স্থাপন করা হয়েছে',
                'subject' => 'নতুন আদেশ নিশ্চিতকরণ',
                'summary' => 'অর্ডারের সংক্ষেপ',
                'title' => 'আদেশ নিশ্চিতকরণ!',
            ],

            'invoiced' => [
                'greeting' => 'আপনার চালান :order_id যেগুলি :created_at তারিখে স্থাপন করা হয়েছে',
                'subject' => 'নতুন চালান নিশ্চিতকরণ',
                'summary' => 'চালানের সংক্ষেপ',
                'title' => 'চালান নিশ্চিতকরণ!',
            ],

            'shipped' => [
                'greeting' => 'আপনার অর্ডার :order_id যেগুলি :created_at তারিখে প্রেরিত হয়েছে',
                'subject' => 'নতুন চালান নিশ্চিতকরণ',
                'summary' => 'চালানের সংক্ষেপ',
                'title' => 'চালান নিশ্চিতকরণ!',
            ],

            'refunded' => [
                'greeting' => 'ফেরত নেওয়া হয়েছে আপনার অর্ডার :order_id যেগুলি :created_at তারিখে',
                'subject' => 'নতুন ফেরত নিশ্চিতকরণ',
                'summary' => 'ফেরতের সংক্ষেপ',
                'title' => 'অর্ডার ফেরত নেওয়া!',
            ],

            'canceled' => [
                'greeting' => 'আপনার অর্ডার :order_id যেগুলি :created_at তারিখে বাতিল হয়েছে',
                'subject' => 'নতুন অর্ডার বাতিল',
                'summary' => 'অর্ডারের সংক্ষেপ',
                'title' => 'অর্ডার বাতিল!',
            ],

            'commented' => [
                'subject' => 'নতুন মন্তব্য যোগ করা হয়েছে',
                'title' => 'আপনার অর্ডার :order_id যেগুলি :created_at তারিখে নতুন মন্তব্য যোগ করা হয়েছে',
            ],

            'billing-address' => 'বিলিং ঠিকানা',
            'carrier' => 'বাহক',
            'contact' => 'যোগাযোগ',
            'discount' => 'ছাড়',
            'excl-tax' => 'কর ব্যতিত: ',
            'grand-total' => 'সর্বমোট',
            'name' => 'নাম',
            'payment' => 'পেমেন্ট',
            'price' => 'মূল্য',
            'qty' => 'পরিমাণ',
            'shipping' => 'শিপিং',
            'shipping-address' => 'শিপিং ঠিকানা',
            'shipping-handling' => 'শিপিং হ্যান্ডলিং',
            'shipping-handling-excl-tax' => 'শিপিং হ্যান্ডলিং (কর ব্যতিত)',
            'shipping-handling-incl-tax' => 'শিপিং হ্যান্ডলিং (কর সহ)',
            'sku' => 'এসকিউ',
            'subtotal' => 'উপসমষ্টি',
            'subtotal-excl-tax' => 'উপসমষ্টি (কর ব্যতিত)',
            'subtotal-incl-tax' => 'উপসমষ্টি (কর সহ)',
            'tax' => 'কর',
            'tracking-number' => 'ট্র্যাকিং নম্বর: :tracking_number',
        ],
    ],

    'rma' => [
        'customer' => [
            'delivered' => 'বিতরণ করা হয়েছে',
            'menu-name' => 'আরএমএ',
            'offer' => 'আপনার প্রথম অর্ডারে ৪০% পর্যন্ত ছাড় পান',
            'rma-qty' => 'RMA পরিমাণ',
            'shop-now' => 'এখনই কেনাকাটা করুন',
            'submit-req' => 'অনুরোধ জমা দিন',
            'title' => 'আরএমএ',
            'undelivered' => 'অপ্রদত্ত',

            'create' => [
                'cancel' => 'বাতিল করুন',
                'create-btn' => 'সংরক্ষণ করুন',
                'enter-order-id' => 'অর্ডার আইডি প্রবেশ করুন',
                'exchange-window' => 'বিনিময় উইন্ডো',
                'heading' => 'নতুন আরএমএ অনুরোধ',
                'image' => 'ছবি',
                'images' => 'ছবি',
                'information' => 'অতিরিক্ত তথ্য',
                'item-ordered' => 'অর্ডার করা আইটেম',
                'no-record' => 'কোনও রেকর্ড পাওয়া যায়নি!',
                'not-allowed' => 'বিচারাধীন অর্ডারের জন্য আরএমএ অনুমোদিত নয়',
                'order-status' => 'অর্ডারের অবস্থা',
                'orders' => 'অর্ডারসমূহ',
                'price' => 'মূল্য',
                'product' => 'পণ্য',
                'product-name' => 'পণ্যের নাম',
                'quantity' => 'পরিমাণ',
                'reason' => 'কারণ',
                'reopen-request' => 'আবার অনুরোধ করুন',
                'resolution' => 'সমাধান নির্বাচন করুন',
                'return-window' => 'ফেরত উইন্ডো',
                'rma-not-available-quotes' => 'আরএমএর জন্য উপলব্ধ নয়',
                'save' => 'সংরক্ষণ করুন',
                'search-order' => 'অর্ডার অনুসন্ধান করুন',
                'sku' => 'এসকিউ',
                'view' => 'আরএমএ দেখুন',
            ],
        ],

        'guest' => [
            'view' => [
                'close-rma' => 'আরএমএ বন্ধ করুন:',
                'order-status' => 'অর্ডারের অবস্থা:',
                'rma-status' => 'আরএমএর অবস্থা:',
                'title' => 'আরএমএ',
            ],

            'create' => [
                'cancel' => 'বাতিল করুন',
                'create-btn' => 'সংরক্ষণ করুন',
                'enter-order-id' => 'অর্ডার আইডি প্রবেশ করুন',
                'heading' => 'নতুন আরএমএ অনুরোধ',
                'image' => 'ছবি',
                'images' => 'ছবি',
                'information' => 'অতিরিক্ত তথ্য',
                'item-ordered' => 'অর্ডার করা আইটেম',
                'not-allowed' => 'বিচারাধীন অর্ডারের জন্য আরএমএ অনুমোদিত নয়',
                'order-status' => 'অর্ডারের অবস্থা',
                'orders' => 'অর্ডারসমূহ',
                'price' => 'মূল্য',
                'product-name' => 'পণ্যের নাম',
                'product' => 'পণ্য',
                'quantity' => 'পরিমাণ',
                'reason' => 'কারণ',
                'reopen-request' => 'আবার অনুরোধ করুন',
                'resolution' => 'সমাধান নির্বাচন করুন',
                'rma-not-available-quotes' => 'আরএমএর জন্য উপলব্ধ নয়',
                'save' => 'সংরক্ষণ করুন',
                'search-order' => 'অর্ডার অনুসন্ধান করুন',
                'sku' => 'এসকিউ',
                'title' => 'আরএমএ',
            ],

            'index' => [
                'create' => 'নতুন আরএমএ অনুরোধ',
                'delete' => 'মুছুন',
                'edit' => 'সম্পাদনা করুন',
                'guest' => 'অতিথি আরএমএ প্যানেল',
                'heading' => 'গ্রাহক আরএমএ প্যানেল',
                'update' => 'আপডেট করুন',
                'view' => 'দেখুন',
            ],
        ],

        'customer-rma-index' => [
            'cancel' => 'বাতিল করুন',
            'create' => 'তৈরি করুন',
            'delete' => 'মুছুন',
            'edit' => 'সম্পাদনা করুন',
            'guest' => 'অতিথি আরএমএ প্যানেল',
            'heading' => 'আরএমএ',
            'update' => 'আপডেট করুন',
            'view' => 'দেখুন',
        ],

        'validation' => [
            'close-rma' => 'নিশ্চিত করুন',
            'information' => 'অতিরিক্ত তথ্য',
            'order-id' => 'অর্ডার নির্বাচন',
            'order-status' => 'অর্ডারের অবস্থা',
            'orders' => 'অর্ডারসমূহ',
            'resolution' => 'সমাধান',
            'select-orders' => 'অর্ডার নির্বাচন করুন',
        ],

        'conversation-texts' => [
            'by' => 'দ্বারা',
            'customer' => 'গ্রাহক',
            'no-record' => 'কোনও রেকর্ড পাওয়া যায়নি!',
            'on' => 'উপর',
            'seller' => 'বিক্রেতা',
        ],

        'default-option' => [
            'others' => 'অন্যান্য',
            'please-select-value' => 'দয়া করে মান নির্বাচন করুন',
            'select-order-status' => 'অর্ডারের অবস্থা নির্বাচন করুন',
            'select-order' => 'অর্ডার নির্বাচন করুন',
            'select-quantity' => 'পরিমাণ নির্বাচন করুন',
            'select-reason' => 'কারণ নির্বাচন করুন',
            'select-resolution' => 'সমাধান নির্বাচন করুন',
            'select-seller' => 'বিক্রেতা নির্বাচন করুন',
        ],

        'view-customer-rma' => [
            'additional-information' => 'অতিরিক্ত তথ্য:',
            'admin' => 'অ্যাডমিন',
            'cancel-order' => 'অর্ডার বাতিল করুন',
            'change-rma-status' => 'আরএমএর অবস্থা পরিবর্তন করুন',
            'close-rma' => 'আরএমএ বন্ধ করুন:',
            'conversations' => 'আলাপচারিতা',
            'guest' => 'অতিথি',
            'heading' => 'আরএমএ বিবরণ',
            'images' => 'ছবি:',
            'items-request' => 'আরএমএর জন্য অনুরোধকৃত আইটেম(গুলি)',
            'items-requested-for-rma' => 'আরএমএর জন্য অনুরোধকৃত আইটেম(গুলি)',
            'order-id' => 'অর্ডার আইডি:',
            'refund-details' => 'ফেরত বিবরণ',
            'refund-offline-btn' => 'অফলাইন ফেরত',
            'refundable-amount' => 'ফেরতযোগ্য পরিমাণ',
            'resolution-type' => 'সমাধানের ধরন:',
            'rma' => 'আরএমএ',
            'save-btn' => 'সংরক্ষণ করুন',
            'send-message-btn' => 'বার্তা পাঠান',
            'send-message' => 'বার্তা পাঠান',
            'status-details' => 'অবস্থার বিবরণ',
            'status-quotes' => 'সমাধান হিসেবে চিহ্নিত করতে সম্মত হন',
            'status-reopen' => 'পুনরায় খোলার জন্য চেক করুন',
            'status' => 'অবস্থা',
            'term' => 'সম্মতি চিহ্ন ক্ষেত্র প্রয়োজনীয়',
            'you' => 'অ্যাডমিন',
        ],

        'view-guest-rma' => [
            'additional-information' => 'অতিরিক্ত তথ্য:',
            'admin' => 'অ্যাডমিন',
            'close-rma' => 'আরএমএ বন্ধ করুন',
            'conversations' => 'আলাপচারিতা',
            'guest' => 'আপনি',
            'images' => 'ছবি',
            'items-request' => 'আরএমএর জন্য অনুরোধকৃত আইটেম(গুলি)',
            'order-id' => 'অর্ডার আইডি:',
            'refund-offline-btn' => 'অফলাইন ফেরত',
            'resolution-type' => 'সমাধানের ধরন:',
            'rma' => 'আরএমএ',
            'save-btn' => 'সংরক্ষণ করুন',
            'send-message-btn' => 'বার্তা পাঠান',
            'send-message' => 'বার্তা পাঠান',
            'status-details' => 'অবস্থার বিবরণ',
            'status-quotes' => 'সমাধান হিসেবে চিহ্নিত করতে সম্মত হন।',
            'status' => 'অবস্থা',
            'term' => 'সম্মতি চিহ্ন ক্ষেত্র প্রয়োজনীয়',
        ],

        'view-guest-rma-content' => [
            'full-amount' => 'পূর্ণ পরিমাণ',
            'order-status' => 'অর্ডারের অবস্থা :',
            'request-on' => 'অনুরোধ প্রদান :',
            'rma-status' => 'আরএমএ অবস্থা :',
        ],

        'view-customer-rma-content' => [
            'admin-status' => 'প্রশাসক অবস্থা:',
            'close-rma' => 'আরএমএ বন্ধ করুন',
            'consignment-no' => 'সমন্বয় নম্বর:',
            'enter-message' => 'বার্তা লিখুন',
            'full-amount' => 'পূর্ণ পরিমাণ',
            'order-details' => 'অর্ডারের বিবরণ',
            'order-status' => 'অর্ডারের অবস্থা :',
            'partial-amount' => 'আংশিক পরিমাণ',
            'refundable-amount' => 'পরিশোধযোগ্য পরিমাণ:',
            'request-on' => 'অনুরোধ প্রদান :',
            'rma-status' => 'আরএমএ অবস্থা :',
            'seller' => 'বিক্রেতা',
            'total-refundable-amount' => 'মোট পরিশোধযোগ্য পরিমাণ:',
        ],

        'table-heading' => [
            'image' => 'ছবি',
            'order-qty' => 'অর্ডার পরিমাণ',
            'price' => 'মূল্য',
            'product-name' => 'পণ্যের নাম',
            'reason' => 'কারণ',
            'resolution-type' => 'সমাধানের ধরন',
            'rma-qty' => 'RMA পরিমাণ',
            'sku' => 'এসকেইউ',
        ],

        'guest-users' => [
            'button-text' => 'লগইন',
            'email' => 'ইমেল',
            'heading' => 'অতিথি লগইন প্যানেল',
            'logout' => 'অতিথি লগআউট',
            'order-id' => 'অর্ডার আইডি',
            'title' => 'অতিথি লগইন',
        ],

        'mail' => [
            'customer-rma-create' => [
                'additional-information' => 'অতিরিক্ত তথ্য :',
                'greeting' => 'আপনি অর্ডার :order_id এর জন্য নতুন RMA অনুরোধ করেছেন।',
                'heading' => 'RMA অনুরোধ',
                'hello' => 'প্রিয় :name',
                'order-id' => 'অর্ডার আইডি :',
                'order-status' => 'অর্ডারের স্থিতি :',
                'requested-rma-product' => 'অনুরোধিত RMA পণ্য:',
                'resolution-type' => 'সমাধানের ধরণ :',
                'rma-id' => 'RMA আইডি :',
                'summary' => 'অর্ডারের RMA সংক্ষিপ্তসারণ',
                'thank-you' => 'ধন্যবাদ',
            ],

            'customer-data-table-heading' => [
                'product-name' => 'পণ্যের নাম',
                'qty' => 'পরিমাণ',
                'reason' => 'কারণ',
                'sku' => 'SKU',
            ],

            'customer-conversation' => [
                'subject' => 'নতুন বার্তা',
                'heading' => 'প্রিয় :name,',
                'message' => 'বার্তা',
                'process' => 'আপনার ফেরতের অনুরোধ প্রক্রিয়াধীন রয়েছে।',
                'quotes' => 'ক্রেতার দিক থেকে একটি নতুন বার্তা রয়েছে',
                'solved' => 'RMA স্থিতি গ্রাহকের দ্বারা সমাধান করা হয়েছে।',
            ],

            'seller-conversation' => [
                'subject' => 'নতুন বার্তা',
                'heading' => 'প্রিয় :name',
                'message' => 'বার্তা',
                'quotes' => 'অ্যাডমিন থেকে একটি নতুন বার্তা এসেছে',
                'title' => 'বার্তা প্রাপ্ত হয়েছে!',
            ],

            'status' => [
                'heading' => 'প্রিয় :name',
                'quotes' => 'আপনার RMA অবস্থা বিক্রেতার দ্বারা পরিবর্তিত হয়েছে',
                'rma-id' => 'RMA আইডি',
                'status-change' => ':id টি বিক্রেতার দ্বারা পরিবর্তিত হয়েছে',
                'status' => 'অবস্থা',
                'title' => 'অবস্থা আপডেট হয়েছে!',
                'your-rma-id' => 'আপনার RMA আইডি',
            ],
        ],

        'status' => [
            'status-name' => [
                'accept' => 'অনুমোদিত',
                'awaiting' => 'ফেরতের অপেক্ষায়',
                'canceled' => 'অনুরোধ বাতিল',
                'declined' => 'অনুরোধ প্রত্যাখ্যাত',
                'dispatched-package' => 'ফেরত পরিবহনে',
                'item-canceled' => 'আইটেম বাতিল করা হয়েছে',
                'not-received-package-yet' => 'এখনো প্যাকেজ পাওয়া যায়নি',
                'pending' => 'পর্যালোচনার অপেক্ষায়',
                'processing' => 'প্রসেসিং',
                'received-package' => 'ফেরত দেওয়া হয়েছে',
                'solved' => 'সমাধান করা হয়েছে',
            ],

            'status-quotes' => [
                'declined-admin' => 'এডমিন দ্বারা RMA অস্বীকৃত হয়েছে।',
                'declined-buyer' => 'ক্রেতা দ্বারা RMA অস্বীকৃত হয়েছে।',
                'solved-by-admin' => 'এডমিন দ্বারা RMA সমাধান করা হয়েছে।',
                'solved' => 'RMA সমাধান করা হয়েছে।',
            ],
        ],

        'response' => [
            'already-cancel' => 'RMA স্ট্যাটাস ইতিমধ্যেই বাতিল করা হয়েছে।',
            'cancel-not-allowed' => 'এই RMA অনুরোধ বাতিল করার অনুমতি নেই।',
            'cancel-success' => 'RMA স্থিতি সফলভাবে বাতিল করা হয়েছে।',
            'close-not-allowed' => 'এই RMA অনুরোধ বন্ধ করার অনুমতি নেই।',
            'create-success' => 'অনুরোধ সফলভাবে তৈরি করা হয়েছে।',
            'creation-error' => 'RMA স্ট্যাটাস আপডেট করা সম্ভব নয় কারণ এই অর্ডারের জন্য চালান তৈরি করা হয়নি।',
            'invalid-item' => 'নির্বাচিত অর্ডার আইটেমটি অবৈধ।',
            'invalid-order' => 'নির্বাচিত অর্ডারটি অবৈধ।',
            'permission-denied' => 'আপনি লগ ইন করেছেন',
            'rma-disabled' => 'এই পণ্যের জন্য RMA অক্ষম করা হয়েছে',
            'reopen-not-allowed' => 'এই RMA অনুরোধ পুনরায় খোলার অনুমতি নেই।',
            'send-message' => ':name সফলভাবে প্রেরণ করা হয়েছে।',
            'update-success' => ':name সফলভাবে আপডেট হয়েছে।',
        ],
    ],

    'eu_withdrawal' => [
        'button' => [
            'withdraw' => 'চুক্তি থেকে প্রত্যাহার',
            'view_existing' => 'আপনার প্রত্যাহার দেখুন',
        ],

        'guest_dropdown' => [
            'link' => 'চুক্তি থেকে প্রত্যাহার',
        ],

        'lookup' => [
            'page_title' => 'চুক্তি থেকে প্রত্যাহার',
            'heading' => 'আপনার চুক্তি থেকে প্রত্যাহার করুন',
            'intro' => 'অর্ডার নম্বর এবং চেকআউটে ব্যবহৃত ইমেল ঠিকানা লিখুন। আমরা আপনাকে প্রত্যাহার দাখিল করার একটি লিঙ্ক পাঠাব।',
            'order_number' => 'অর্ডার নম্বর',
            'email' => 'ইমেল ঠিকানা',
            'submit' => 'আমাকে প্রত্যাহার লিঙ্ক পাঠান',
            'sent_notice' => 'যদি কোনো মিল থাকা অর্ডার বিদ্যমান থাকে, আমরা সেই ঠিকানায় একটি প্রত্যাহার লিঙ্ক ইমেল করেছি। এটি ২৪ ঘণ্টার জন্য বৈধ থাকবে।',
            'legal_note' => 'EU Directive 2011/83/EU (সংশোধিত) অনুযায়ী আপনার ১৪ দিনের মধ্যে চুক্তি থেকে প্রত্যাহারের আইনি অধিকার রয়েছে। প্রত্যাহারের অন্যান্য মাধ্যম — আমাদের গ্রাহক সেবায় লিখিত বিবৃতি প্রেরণ সহ — বৈধ থাকবে।',
        ],

        'form' => [
            'page_title' => 'আপনার প্রত্যাহার নিশ্চিত করুন',
            'heading' => 'চুক্তি থেকে প্রত্যাহার নিশ্চিত করুন',
            'legal_notice_title' => 'এটি একটি আইনত বাধ্যতামূলক ঘোষণা',
            'legal_effect' => 'এই ফর্ম জমা দিয়ে আপনি অর্ডার #:order_id এর জন্য আপনার আইনি প্রত্যাহারের অধিকার প্রয়োগ করছেন। জমা দেওয়ার মুহূর্ত থেকে ঘোষণাটি আইনত কার্যকর হবে।',
            'order_summary' => 'অর্ডার',
            'order_total' => 'মোট',
            'order_items' => 'আইটেম',
            'placed_on' => ':date তারিখে স্থাপিত',
            'reason_label' => 'কারণ',
            'reason_optional' => '(ঐচ্ছিক)',
            'reason_help' => 'কারণ দেওয়া বাধ্যতামূলক নয়। আপনি এখানে যা লিখবেন তা প্রত্যাহার প্রমাণে রেকর্ড করা হবে এবং বিক্রেতার কাছে পাঠানো হবে।',
            'reason_placeholder' => 'উদাহরণ: "মন পরিবর্তন করেছি", "আইটেম বর্ণনার মতো নয়", অথবা ফাঁকা রাখুন।',
            'submit' => 'প্রত্যাহার জমা দিন',
            'cancel' => 'বাতিল করে ফিরে যান',
        ],

        'confirmation' => [
            'page_title' => 'প্রত্যাহার রসিদ',
            'heading' => 'আপনার প্রত্যাহার গৃহীত হয়েছে',
            'heading_received' => 'আপনার প্রত্যাহার গৃহীত হয়েছে',
            'heading_refunded' => 'আপনার ফেরত জারি করা হয়েছে',
            'heading_declined' => 'আপনার প্রত্যাহার প্রত্যাখ্যাত হয়েছে',
            'intro' => 'আমরা আপনার ঘোষণা রেকর্ড করেছি। এই পৃষ্ঠা এবং আমরা যে ইমেলটি পাঠিয়েছি উভয়ই স্থায়ী মাধ্যমে নিশ্চিতকরণ হিসেবে কাজ করে।',
            'intro_received' => 'আমরা আপনার ঘোষণা রেকর্ড করেছি। এই পৃষ্ঠা এবং আমরা যে ইমেলটি পাঠিয়েছি উভয়ই স্থায়ী মাধ্যমে নিশ্চিতকরণ হিসেবে কাজ করে।',
            'intro_refunded' => 'বিক্রেতা আপনার ফেরত প্রক্রিয়া করেছেন। প্রতিদান শীঘ্রই আপনার মূল পেমেন্ট পদ্ধতিতে পৌঁছাবে।',
            'intro_declined' => 'এই ঘোষণার অধীনে বিক্রেতা ফেরতের আপনার অধিকার বিতর্কিত করেছেন। প্রদত্ত কারণ এবং আপনার অবশিষ্ট বিকল্পের জন্য নীচের বিবরণ দেখুন।',
            'received_at' => 'গ্রহণের সময়',
            'reference' => 'রেফারেন্স',
            'copy_reference' => 'রেফারেন্স কপি করুন',
            'reference_copied' => 'রেফারেন্স ক্লিপবোর্ডে কপি করা হয়েছে।',
            'order' => 'অর্ডার',
            'email' => 'ইমেল',
            'reason' => 'কারণ',
            'status' => 'অবস্থা',
            'status_received' => 'গৃহীত',
            'status_refunded' => 'ফেরত',
            'status_declined' => 'প্রত্যাখ্যাত',
            'print' => 'প্রিন্ট',
            'next_steps_title' => 'এর পরে কী হবে',
            'step_received' => 'আপনার ঘোষণা গৃহীত ও রেকর্ড করা হয়েছে।',
            'step_email' => 'আপনার ইনবক্সে নিশ্চিতকরণ ইমেল পাঠানো হয়েছে।',
            'step_refund' => '১৪ দিনের মধ্যে ফেরত জারি করা হবে।',
            'step_refund_done' => 'ফেরত জারি করা হয়েছে।',
            'step_declined' => 'বিক্রেতা দ্বারা প্রত্যাহার প্রত্যাখ্যাত।',
            'declined_reason_label' => 'বিক্রেতা প্রদত্ত কারণ',
            'declined_notice' => 'EU ভোক্তা আইনের অধীনে আপনার অন্যান্য সমস্ত আইনি প্রতিকার বহাল রয়েছে। আপনি এই সিদ্ধান্তের সাথে দ্বিমত হলে, সরাসরি বিক্রেতার সাথে যোগাযোগ করুন।',
            'refund_notice' => 'অন্যথায় স্পষ্টভাবে সম্মত না হলে, আমরা মূল লেনদেনের জন্য আপনি যে পেমেন্ট পদ্ধতি ব্যবহার করেছেন সেই একই পদ্ধতিতে প্রতিদান করি।',
            'email_sent' => 'নিশ্চিতকরণ ইমেল পাঠানো হয়েছে।',
            'email_pending' => 'আমরা এই মুহূর্তে নিশ্চিতকরণ ইমেল পাঠাতে পারিনি; আমাদের দল এটি অনুসরণ করবে। অনুগ্রহ করে এই পৃষ্ঠাটি আপনার রসিদ হিসেবে সংরক্ষণ করুন।',
            'durable_medium_notice' => 'এই পৃষ্ঠাটি আপনার রসিদ। আপনি এটি সংরক্ষণ বা প্রিন্ট করতে পারেন।',
        ],

        'emails' => [
            'confirmation' => [
                'subject' => 'আপনার প্রত্যাহারের নিশ্চিতকরণ — অর্ডার :order_id',
                'title' => 'প্রত্যাহার গৃহীত',
                'title_received' => 'প্রত্যাহার গৃহীত',
                'title_refunded' => 'ফেরত জারি করা হয়েছে',
                'title_declined' => 'প্রত্যাহার প্রত্যাখ্যাত',
                'greeting' => 'নমস্কার,',
                'intro' => 'আমরা অর্ডার #:order_id এর চুক্তি থেকে আপনার প্রত্যাহার ঘোষণা পেয়েছি। আইনত কার্যকর গ্রহণের সময় নীচে রেকর্ড করা হয়েছে।',
                'intro_received' => 'আমরা অর্ডার #:order_id এর চুক্তি থেকে আপনার প্রত্যাহার ঘোষণা পেয়েছি। আইনত কার্যকর গ্রহণের সময় নীচে রেকর্ড করা হয়েছে।',
                'intro_refunded' => 'অর্ডার #:order_id এর জন্য আপনার ফেরত জারি করা হয়েছে। প্রতিদান শীঘ্রই আপনার মূল পেমেন্ট পদ্ধতিতে পৌঁছাবে।',
                'intro_declined' => 'এই ঘোষণার অধীনে অর্ডার #:order_id এর জন্য ফেরতের আপনার অধিকার বিক্রেতা বিতর্কিত করেছেন। প্রদত্ত কারণ নীচে রয়েছে।',
                'summary' => 'প্রত্যাহার সারাংশ',
                'received_at' => 'গ্রহণের সময়',
                'reference' => 'রেফারেন্স',
                'order' => 'অর্ডার',
                'email' => 'ইমেল',
                'reason' => 'প্রদত্ত কারণ',
                'declined_reason' => 'প্রত্যাখ্যানের কারণ',
                'refund_notice' => 'যেখানে ফেরত প্রাপ্য, আমরা উপরের তারিখ থেকে ১৪ দিনের মধ্যে আপনি মূল লেনদেনের জন্য ব্যবহৃত একই পেমেন্ট পদ্ধতিতে প্রতিদান করি, অন্যথায় স্পষ্টভাবে সম্মত না হলে।',
                'footer' => 'এই বার্তাটি Directive 2011/83/EU এর Article 11a (Directive (EU) 2023/2673 দ্বারা সংশোধিত) দ্বারা প্রয়োজনীয় স্থায়ী মাধ্যমে নিশ্চিতকরণ। অনুগ্রহ করে এটি আপনার রেকর্ডের জন্য সংরক্ষণ করুন।',
            ],

            'guest_link' => [
                'subject' => 'আপনার প্রত্যাহার লিঙ্ক',
                'title' => 'আপনার প্রত্যাহার লিঙ্ক',
                'intro' => 'অর্ডার :order_id এর প্রত্যাহার দাখিল করতে নীচের লিঙ্কটি ব্যবহার করুন। লিঙ্কটি ২৪ ঘণ্টার জন্য বৈধ।',
                'button' => 'প্রত্যাহার ফর্ম খুলুন',
                'expiry' => 'লিঙ্কটি ২৪ ঘণ্টায় মেয়াদ শেষ হবে। আপনি যদি এই ইমেল অনুরোধ না করে থাকেন, তবে এটি নিরাপদে উপেক্ষা করতে পারেন।',
            ],
        ],
    ],
];
