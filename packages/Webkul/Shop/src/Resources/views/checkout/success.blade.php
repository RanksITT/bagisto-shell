...
>
	...
	<div class="container mt-8 px-[60px] max-lg:px-8">
		<div class="grid place-items-center gap-y-5 max-md:gap-y-2.5">
			...
			<p class="text-xl max-md:text-sm">
				@if (auth()->guard('customer')->user())
					@lang('shop::app.checkout.success.order-id-info', [
						'order_id' => '<a class="text-darkBlue" href="'.route('shop.customers.account.orders.view', $order->id).'">'.$order->increment_id.'</a>'
					])
				@else
			...
			<p class="text-xl text-mutedBlue max-md:text-center max-md:text-xs">
			...
			<a href="{{ route('shop.home.index') }}">
				<div class="w-max cursor-pointer rounded-2xl bg-darkBlue px-11 py-3 text-center text-base font-medium text-navyBlue max-md:rounded-lg max-md:px-6 max-md:py-1.5">
